...
export class BabylonMeshService {
  ...
  private parseCssColor(colorInput: string | undefined): { color: Color3; alpha: number } {
    if (!colorInput) {
      ...
      if (parts.length >= 3) {
        const channelFromString = (value: string): number => {
          if (value.endsWith('%')) {
            ...
            return Number.isNaN(percent) ? 0 : Math.max(0, Math.min(255, percent * 2.55));
          }
        ...
        const alpha = parts[3] !== undefined ? Math.max(0, Math.min(1, parseFloat(parts[3]))) : 1;
    ...
    console.warn(`⚠️ Unsupported color format "${colorInput}". Falling back to black.`);
    return { color: new Color3(0, 0, 0), alpha: 1 };
  }
  ...
  updateMeshWithBorderRadius(
    ...
    try {
      ...
      if (borderWidth > 0) {
        ...
        const borderMesh = targetScene.getMeshByName(borderMeshName) as Mesh | null;
        ...
        if (borderMesh) {
          ...
          for (let i = 0; i < 4; i++) {
            ...
            if (legacyMesh) {
              console.log(`⚠️ Found legacy rectangular border mesh: ${legacyName} – not updated for border radius`);
            }
          }
        }
      }
