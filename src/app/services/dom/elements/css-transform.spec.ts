...
describe('CSS transform coordinates', () => {
  const projection = (scale: number) => ({
    projectCssLocalPoint: ({ x, y }: { x: number; y: number }, z = 0) => ({
      x: x * scale,
      y: -y * scale,
      z,
    }),
    projectCssLength: (value: number) => value * scale,
  });
  ...
  it('maps positive CSS right/down translations at the render boundary', () => {
    ...
    expect(cssTranslationToRenderOffset(transform, projection(2))).toEqual({ x: 3, y: -5, z: 0 });
    expect(cssTranslationToRenderOffset(transform, projection(4))).toEqual({ x: 6, y: -10, z: 0 });
  });
  ...
  it('preserves the same CSS displacement when projected at different DPR scales', () => {
    ...
    const dpr1 = cssTranslationToRenderOffset(transform, projection(2));
    const dpr2 = cssTranslationToRenderOffset(transform, projection(4));
