...
const observations = group.observations.map(observation => {
  ...
  for (const side of ['reference', 'astylar']) {
    ...
    while (node) {
      ...
      chain.push({ key: node.key, parent: node.parent, attributes: node.attributes,
        authored: node.authored, styles: Object.fromEntries(properties.map(property =>
...
console.log(JSON.stringify({ kind: 'tooltip-position-ancestry-inspection', population: { file, sha256: hash(bytes) },
  observations, canonicalAttributionChanged: false, rendererCauseProven: false }, null, 2));
