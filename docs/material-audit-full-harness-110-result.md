...
| --- | --- | --- |
| 13, 51 | Button fixed-width and button-request historical integration | The corrected width test passed with all original controls. The request rerun caught a copied population-count error (its original evidence contains 566 observations, not the width test's 1,200); the count is corrected and the full request test must pass. Both guards independently authenticate exactly 18 later classifications before requiring complete equality elsewhere. See `material-button-fixed-width-audit.md`. |
