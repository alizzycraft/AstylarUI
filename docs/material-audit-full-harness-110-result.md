...
| 740 | Reviewed-authoring historical integration | Corrected full file passes 2/2, including detached-evidence and inflated-claim controls. All 2,944 scalar rows remain intact; exactly 18 later source-bound groups / 758 observations are accounted for, and the other 2,738 complete rows are unchanged. The original failing diagnostic is retained. See `material-button-fixed-width-audit.md`. |
| 757 | Root initial-style evidence index | Complete replay verifies 2,311 cases / 30,043 observations / 468 groups, unchanged membership and non-receipt evidence. Exactly two receipts changed (builder and source-inventory test); root calculations and assertions are unchanged. See `material-root-initial-receipt-diagnostic.md`. Refresh only verified metadata and rerun the original file. |
