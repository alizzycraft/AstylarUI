...
The [own-text weight/tracking proof](material-leaf-weight-tracking-stages.md)
adds 304 source observations and passes 7/7 focused/inventory checks in
8,675.5415ms, including all original observations and 48 rejection controls.
Its canonical membership join remains pending. The [downstream motion refresh](material-reviewed-motion-receipt-refresh.md)
passes 3/3 with only three dependency hashes changed and all 64 observations
conserved. The two historical gap tests now contain exact later-metadata
...
At that snapshot the failure had not been independently diagnosed; the
explicit-gap diagnosis could not be generalized without checking its records.
The running harness remains unaccepted, and the new suite does not repair
either historical conservation test.

The subsequent [exact gap-review diagnostic](material-gap-review-later-conservation.md)
now explains failure 111 independently: 32 later caret groups / 796 observations
change only review metadata. All 2,173 scalar rows and 2,105 other complete rows
are unchanged, including 64 unresolved dialog motion and 155 pending caret
observations. The diagnostic retains the original failing assertion and exits
1 after its evidence checks pass. The original test remains unmodified; this
does not turn the historical full run into a pass.
...
accounts for all 78 slider cases and independently demonstrates the eight disabled
visual-owner opacity differences. Its two tests plus the four inventory tests
pass **6/6**, exit **0**, with eighteen rejection controls. This is unequal-input
evidence, not a slider drag fix, canonical promotion or full harness acceptance.
