...
Current supported intrinsic rows are fixed pixel rows and `auto` rows whose item contributions can be measured recursively. Phase 6 will extend this through focused evidence. `auto-fit`, `auto-fill`, named lines/areas, arbitrary placement, spanning, subgrid, and masonry remain out of scope. Unsupported intrinsic combinations must return an explicit unsupported result rather than silently using zero-sized tracks.
...
Continue from fresh baselines for width-constrained wrapped text contributions, nested block/control/image content, implicit rows, mixed intrinsic tracks, intrinsic keywords and bounds, and fixed-count `repeat()`. Remove dashboard row-height declarations only after the underlying contribution behavior is covered and green.
