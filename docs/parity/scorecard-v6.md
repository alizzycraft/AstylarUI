...
Continue from fresh baselines for nested block/control/image content, implicit rows, mixed intrinsic tracks, intrinsic keywords and bounds, and fixed-count `repeat()`. Remove dashboard row-height declarations only after the underlying contribution behavior is covered and green.
