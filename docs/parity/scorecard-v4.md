...
The project dashboard title and metadata now derive their heights from typography rather than explicit `28px` and `20px` declarations, with unchanged SSIM at all three viewports. An attempted removal of the responsive summary-label heights exposed a separate flex-item centering defect; those declarations remain until that flex behavior receives its own focused repair.
