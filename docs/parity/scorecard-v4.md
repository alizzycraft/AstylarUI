...
The next increment should be chosen from fresh browser/Astylar baselines, prioritizing the behavior that removes the largest amount of representative-app workaround sizing. Likely candidates are intrinsic form-control sizing, then content-driven non-text flex containers and grid cross sizes. Do not remove an application workaround until its underlying general behavior has focused regression coverage and passes the full corpus.
