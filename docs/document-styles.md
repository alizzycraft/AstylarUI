...
The paired `tailwind-loaded-utilities` gate runs at DPR 1 and 2, measures local
title/control sharpness, and drives its utility-authored overflow region to the
bottom reachability boundary. That rounded-scroll composition permits a local
`2.1px` extent tolerance; every existing scrolling fixture remains exact.
