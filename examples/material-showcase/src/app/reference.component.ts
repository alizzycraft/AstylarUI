    ...
    <main class="frame" [class]="densityClass()" [class.benchmark]="benchmarkMode" [class.dark]="store.theme().mode === 'dark'" [style.--primary]="store.tokens().primary" [style.--surface]="store.tokens().surface" [style.--scale]="store.tokens().typographyScale"
      ...
      <section class="demo" [id]="family() + '-root'" [attr.aria-label]="family() + ' showcase'" [style.border-radius.px]="24 * store.tokens().cornerScale">
        @switch (family()) {
          ...
          @case ('list') { <mat-list id="list-primary"><mat-list-item>Inbox</mat-list-item><mat-list-item>Archive</mat-list-item></mat-list> }
          @case ('table') { <table id="table-primary" mat-table [dataSource]="rows"><ng-container matColumnDef="name"><th mat-header-cell *matHeaderCellDef>Name</th><td mat-cell *matCellDef="let row">{{row.name}}</td></ng-container><tr mat-header-row *matHeaderRowDef="['name']"></tr><tr mat-row *matRowDef="let row; columns:['name']"></tr></table> }
          @case ('sort') { <div id="sort-primary" matSort [matSortDirection]="store.state().sortDirection" (matSortChange)="store.patchState({sortDirection:$event.direction === 'desc' ? 'desc' : 'asc'})"><div mat-sort-header="name">Sort by name</div></div> }
          ...
          @case ('tree') { <mat-tree id="tree-primary" [dataSource]="treeData" [childrenAccessor]="childrenAccessor"><mat-tree-node *matTreeNodeDef="let node">{{ node.name }}</mat-tree-node></mat-tree> }
          @case ('form-field') { <mat-form-field id="form-field-primary"><mat-label>Project name</mat-label><input id="form-field-control" matInput aria-label="Project name" value="Atlas" [disabled]="store.state().disabled" [errorStateMatcher]="errorStateMatcher"><mat-hint>Public label</mat-hint>@if(store.state().error){<mat-error>Project name is required</mat-error>}</mat-form-field> }
          @case ('input') { <mat-form-field id="input-primary"><mat-label>Email</mat-label><input id="input-control" matInput aria-label="Email" type="email" value="team@example.com" [disabled]="store.state().disabled" [errorStateMatcher]="errorStateMatcher"></mat-form-field> }
          @case ('autocomplete') { <mat-form-field id="autocomplete-primary"><mat-label>City</mat-label><input id="autocomplete-control" matInput aria-label="City" [matAutocomplete]="auto" [disabled]="store.state().disabled" [errorStateMatcher]="errorStateMatcher"><mat-autocomplete #auto="matAutocomplete"><mat-option value="Cape Town">Cape Town</mat-option><mat-option value="Johannesburg">Johannesburg</mat-option></mat-autocomplete></mat-form-field> }
          @case ('checkbox') { <mat-checkbox id="checkbox-primary" [checked]="store.state().selected" [disabled]="store.state().disabled" (change)="store.patchState({selected:$event.checked})">Include archived</mat-checkbox> }
          @case ('radio') { <mat-radio-group id="radio-primary" [value]="store.state().selected ? 'team' : 'solo'" [disabled]="store.state().disabled" (change)="store.patchState({selected:$event.value === 'team'})"><mat-radio-button value="solo">Solo</mat-radio-button><mat-radio-button value="team">Team</mat-radio-button></mat-radio-group> }
          @case ('select') { <mat-form-field id="select-primary"><mat-label>Plan</mat-label><mat-select id="select-control" aria-label="Plan" [value]="store.state().selected ? 'team' : 'solo'" [disabled]="store.state().disabled" [attr.aria-disabled]="store.state().disabled" [attr.aria-invalid]="store.state().error" (selectionChange)="store.patchState({selected:$event.value === 'team'})"><mat-option value="solo">Solo</mat-option><mat-option value="team">Team</mat-option></mat-select></mat-form-field> }
          ...
          @case ('slide-toggle') { <mat-slide-toggle id="slide-toggle-primary" [checked]="store.state().selected" [disabled]="store.state().disabled" (change)="store.patchState({selected:$event.checked})">Automatic updates</mat-slide-toggle> }
          @case ('datepicker') { <mat-form-field id="datepicker-primary"><mat-label>Due date</mat-label><input id="datepicker-control" matInput aria-label="Due date" [matDatepicker]="picker" [disabled]="store.state().disabled" [errorStateMatcher]="errorStateMatcher"><mat-datepicker-toggle matIconSuffix [for]="picker"/><mat-datepicker #picker/></mat-form-field> }
          @case ('timepicker') { <mat-form-field id="timepicker-primary"><mat-label>Meeting time</mat-label><input id="timepicker-control" matInput aria-label="Meeting time" [matTimepicker]="time" [disabled]="store.state().disabled" [errorStateMatcher]="errorStateMatcher"><mat-timepicker-toggle matIconSuffix [for]="time"/><mat-timepicker #time/></mat-form-field> }
          @case ('button-toggle') { <mat-button-toggle-group id="button-toggle-primary" [value]="store.state().selected ? 'grid' : 'list'" (change)="store.patchState({selected:$event.value === 'grid'})"><mat-button-toggle value="list">List</mat-button-toggle><mat-button-toggle value="grid">Grid</mat-button-toggle></mat-button-toggle-group> }
          ...
          @case ('tabs') { <mat-tab-group id="tabs-primary"><mat-tab label="Overview">Overview content</mat-tab><mat-tab label="Activity">Activity content</mat-tab></mat-tab-group> }
          @case ('stepper') { <mat-stepper id="stepper-primary"><mat-step label="Details">Project details</mat-step><mat-step label="Review">Review changes</mat-step></mat-stepper> }
          @case ('expansion') { <mat-expansion-panel id="expansion-primary" [expanded]="store.state().open" [disabled]="store.state().disabled"><mat-expansion-panel-header><mat-panel-title>Advanced settings</mat-panel-title></mat-expansion-panel-header><p>Additional options.</p></mat-expansion-panel> }
