    ...
    <main class="frame" [class]="densityClass()" [class.benchmark]="benchmarkMode" [class.dark]="store.theme().mode === 'dark'" [style.--primary]="store.tokens().primary" [style.--surface]="store.tokens().surface" [style.--scale]="store.tokens().typographyScale"
      ...
      <section class="demo" [id]="family() + '-root'" [attr.aria-label]="family() + ' showcase'" [style.border-radius.px]="24 * store.tokens().cornerScale">
        @switch (family()) {
          ...
          @case ('tabs') { <mat-tab-group id="tabs-primary"><mat-tab><ng-template mat-tab-label><span id="tab-overview">Overview</span></ng-template><span id="tab-panel">Overview content</span></mat-tab><mat-tab><ng-template mat-tab-label><span id="tab-activity">Activity</span></ng-template>Activity content</mat-tab></mat-tab-group> }
          @case ('stepper') { <mat-stepper id="stepper-primary"><mat-step><ng-template matStepLabel><span id="step-details-text">Details</span></ng-template><span id="stepper-content">Project details</span></mat-step><mat-step><ng-template matStepLabel><span id="step-review-text">Review</span></ng-template>Review changes</mat-step></mat-stepper> }
          @case ('expansion') { <mat-expansion-panel id="expansion-primary" [expanded]="store.state().open" [disabled]="store.state().disabled"><mat-expansion-panel-header><mat-panel-title id="expansion-title">Advanced settings</mat-panel-title></mat-expansion-panel-header><p>Additional options.</p></mat-expansion-panel> }
