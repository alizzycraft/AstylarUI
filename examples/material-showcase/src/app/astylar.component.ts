...
export class AstylarShowcaseComponent {
  ...
  private readonly focusVisibleId = signal<string | undefined>(undefined);
  private pointerDownId?: string;
      ...
      handlers: new Proxy({}, {
        get: (_target, id: string) => ({
          pointerdown: (event: AstylarEvent) => this.zone.run(() => {
            ...
            this.pointerDownId = this.canonicalFocusTarget(event.targetId);
          ...
          focus: (event: AstylarEvent) => this.zone.run(() => {
            this.recordEvent(event);
            const focusTarget = this.canonicalFocusTarget(event.targetId);
            if (this.authoredTreeDependsOnFocus(focusTarget)) {
              this.focusedId.set(focusTarget);
              this.focusVisibleId.set(this.pointerDownId === focusTarget ? undefined : focusTarget);
            }
          ...
          blur: (event: AstylarEvent) => this.zone.run(() => {
            this.recordEvent(event);
            const focusTarget = this.canonicalFocusTarget(event.targetId);
            if (this.focusedId() === focusTarget) this.focusedId.set(undefined);
            if (this.focusVisibleId() === focusTarget) this.focusVisibleId.set(undefined);
          }),
  ...
  protected mounted(surface: AstylarSurface): void {
    ...
    if (typeof window !== 'undefined' && window.parent !== window) {
      window.parent.postMessage({ type: 'showcase:ready' }, window.location.origin);
    }
  ...
  private handleClick(id: string, event: AstylarEvent): void {
    ...
    if (id === 'autocomplete-option-cape-town' || id === 'autocomplete-option-johannesburg') {
      this.fieldValues.update((values) => ({
        ...values,
        autocomplete: id.includes('cape-town') ? 'Cape Town' : 'Johannesburg',
      }));
      this.store.patchState({ open: false });
    }
    if (id === 'select-option-solo' || id === 'select-option-team') {
      this.store.patchState({ selected: id === 'select-option-team', open: false });
    }
    ...
    const chipIndex = Number(id.match(/^chip-(\d+)$/)?.[1]);
    ...
    if (id === 'timepicker-icon') {
      this.store.patchState({ open: !this.store.state().open });
    }
    if (id === 'timepicker-control') this.store.patchState({ open: true });
  ...
  private canonicalFocusTarget(elementId: string | undefined): string | undefined {
    const fieldFamily = ['form-field', 'input', 'autocomplete', 'select', 'datepicker', 'timepicker']
      .find((family) => elementId === `${family}-primary`);
    return fieldFamily ? `${fieldFamily}-control` : elementId;
  }
  ...
  private buildSiteData(family: MaterialFamily): SiteData {
    ...
    return {
      ...
      styles: [
        ...
        { selector: '.radio-option', position: 'absolute', top: '-10px', height: '40px', display: 'flex', alignItems: 'center', color: theme.onSurface, cursor: 'pointer' },
        { selector: '#radio-solo', left: '0', width: '68px' },
        { selector: '#radio-team', left: '73px', width: '80px' },
        { selector: '.radio-state-layer', position: 'absolute', zIndex: '1', top: '0', left: '-10px', width: '40px', height: '40px', borderRadius: '20px', background: 'transparent' },
        { selector: '.radio-option:hover .radio-state-layer', background: alphaHex(theme.onSurface, .08) },
        { selector: '.radio-option:active .radio-state-layer', background: alphaHex(theme.onSurface, .12) },
        { selector: '.radio-option:hover .radio-state-layer.selected', background: alphaHex(theme.primary, .08) },
        { selector: '.radio-option:active .radio-state-layer.selected', background: alphaHex(theme.primary, .12) },
        { selector: '.radio-ring', position: 'relative', zIndex: '2', width: '20px', height: '20px', boxSizing: 'border-box', borderWidth: '2px', borderStyle: 'solid', borderColor: theme.onSurface, borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' },
        ...
        { selector: '.radio-label', position: 'relative', zIndex: '2', marginLeft: '8px', fontSize: '14px', verticalAlign: 'middle' },
        { selector: '#slide-toggle-primary', position: 'relative', width: '179px', height: '32px', alignSelf: 'flex-start', cursor: 'pointer' },
        ...
        { selector: '.switch-minus', width: '12px', height: '2px', flexShrink: '0', borderRadius: '1px', background: theme.surfaceContainer },
        ...
        { selector: '#button-toggle-one-label, #button-toggle-two-label', lineHeight: `${theme.density <= -5 ? 24 : 40}px`, ...(theme.density <= -5 ? { padding: '.5px 0 0' } : {}) },
        ...
        { selector: '.button-toggle-option:hover', background: mixHex(theme.surfaceContainer, theme.onSurface, .08) },
        { selector: '.button-toggle-option:active', background: mixHex(theme.surfaceContainer, theme.onSurface, .12) },
        { selector: '.button-toggle-option.selected:hover', background: mixHex('#eadef7', '#4b4357', .08) },
        { selector: '.button-toggle-option.selected:active', background: mixHex('#eadef7', '#4b4357', .12) },
        { selector: '#menu-primary', width: '120px' },
        { selector: '#menu-popup', position: 'absolute', top: `${theme.density <= -5 ? 54 : theme.density < 0 ? 58 : 69}px`, left: '28px', width: '112px', height: `${theme.density <= -5 ? 111 : theme.density < 0 ? 110 : 112}px`, boxSizing: 'border-box', padding: '8px 0', borderRadius: '4px', background: '#f2ecf1', boxShadow: '0 3px 1px -2px rgba(0,0,0,.2), 0 2px 2px rgba(0,0,0,.14), 0 1px 5px rgba(0,0,0,.12)', zIndex: '50' },
        { selector: '#menu-rename, #menu-delete', width: '112px', height: '48px', boxSizing: 'border-box', padding: '0 12px', borderWidth: '0', background: 'transparent', color: '#1d1b20', display: 'flex', alignItems: 'center', textAlign: 'left', fontSize: '14px', fontWeight: '500', lineHeight: '20px' },
        { selector: '.menu-option-label', marginBottom: `${theme.density <= -5 ? 4 : 0}px` },
        { selector: '#menu-rename:hover, #menu-delete:hover', background: mixHex('#f2ecf1', '#1d1b20', .08) },
        { selector: '#menu-rename:active, #menu-delete:active', background: mixHex('#f2ecf1', '#1d1b20', .12) },
        { selector: '#menu-primary:hover', background: state.open ? theme.primary : mixHex(theme.primary, theme.onPrimary, .08) },
        ...
        { selector: '.material-button:focus', boxShadow: '0 0 0 1px rgba(0,0,0,0)' },
        ...
        { selector: '.field-surface.active', boxShadow: 'none' },
        { selector: '.field-active-line', position: 'absolute', left: '0', top: `${theme.density === 0 ? 56 : theme.density <= -5 ? 35 : 48}px`, width: '100%', height: '2px', background: state.error ? theme.error : theme.primary, pointerEvents: 'none' },
        ...
        { selector: '.select-control, .select-control:focus', paddingRight: '32px', cursor: 'pointer', boxShadow: 'none', color: 'transparent', caretColor: 'transparent' },
        ...
        { selector: '.select-popup', position: 'absolute', top: `${theme.density === 0 ? 56 : 48}px`, left: '0', right: '0', width: 'auto', height: `${theme.density === 0 ? 112 : 104}px`, boxSizing: 'border-box', padding: '8px 0', background: '#f2ecf1', boxShadow: '0 2px 6px rgba(0,0,0,0.24)', zIndex: '60' },
        { selector: '.select-option', position: 'relative', width: '100%', height: '48px', boxSizing: 'border-box', padding: '0 16px', display: 'flex', alignItems: 'center', color: '#1d1b20', fontSize: '16px' },
        ...
        { selector: '.autocomplete-popup', top: `${theme.density === 0 ? 56 : 50}px` },
        { selector: '.autocomplete-popup .select-option', padding: '0 16px' },
        ...(theme.density === -2 ? [
          { selector: '.field-input-region.compact-filled-input-region', top: '10px' },
          { selector: '.select-popup', top: '49px', left: '0', right: '0', width: 'auto', height: '112px' },
          { selector: '.select-popup .select-option', padding: '0 16px' },
        ...
        ...(theme.density <= -5 ? [
          { selector: '.select-popup', top: '46px', left: '0', right: '0', width: 'auto' },
        ...
        { selector: '.expansion-panel', position: 'relative', width: '100%', height: `${state.open ? theme.density === 0 ? 136 : theme.density <= -5 ? 120 : 128 : theme.density === 0 ? 48 : theme.density <= -5 ? 36 : 40}px`, borderRadius: '12px', background: theme.surface, boxShadow: theme.density <= -5 ? '0 3px 1px -2px rgba(0,0,0,0.2), 0 2px 2px 0 rgba(0,0,0,0.14), 0 1px 5px 0 rgba(0,0,0,0.12)' : '0 1px 2px #00000055' },
        { selector: '.expansion-trigger', width: '100%', height: `${state.open ? theme.density === 0 ? 64 : theme.density <= -5 ? 52 : 56 : theme.density === 0 ? 48 : theme.density <= -5 ? 36 : 40}px`, boxSizing: 'border-box', padding: `0 ${theme.density <= -5 ? 25 : 24}px`, borderWidth: '0', display: 'flex', alignItems: 'center', background: 'transparent', color: theme.onSurface, fontWeight: '500', textAlign: 'left', cursor: 'pointer' },
        { selector: '.expansion-trigger:hover', background: state.open ? 'transparent' : mixHex(theme.surface, theme.onSurface, .08) },
        ...
        ...(family === 'expansion' && theme.density <= -5 ? [{ selector: '.expansion-title', padding: '0', fontSize: '16px' }] : []),
        ...
        { selector: '.expansion-content-label', position: 'absolute', top: '-1px', left: '24px', textAlign: 'left' },
        ...
        { selector: '.toolbar-title', width: '192.15625px', height: '28px', marginLeft: '16px', flexShrink: '0', whiteSpace: 'nowrap', fontFamily: 'Roboto', fontSize: '22px', fontWeight: '400', lineHeight: '28px' },
        { selector: '.toolbar-action', width: '65.140625px', height: `${theme.density === 0 ? 40 : theme.density <= -5 ? 24 : 28}px`, boxSizing: 'border-box', margin: '0 16px 0 auto', padding: '0 12px', flexShrink: '0', display: 'flex', alignItems: 'center', justifyContent: 'center', borderWidth: '0', borderRadius: '20px', background: 'transparent', color: theme.onSurface, fontFamily: 'Roboto', fontSize: '14px', fontWeight: '500', lineHeight: `${theme.density === 0 ? 40 : theme.density <= -5 ? 24 : 28}px`, letterSpacing: '.096px', textAlign: 'center', verticalAlign: 'middle', cursor: 'pointer' },
        { selector: '.toolbar-action', mediaMaxWidth: '500px', width: '64px' },
        ...
        { selector: '.grid-tile-label', position: 'absolute', top: `${theme.density <= -5 ? devicePixelRatio >= 1.5 ? 30.5 : 31 : theme.typographyScale > 1 ? 29 : 31}px`, left: '0', width: '100%', textAlign: 'center', whiteSpace: 'nowrap' },
        ...(family === 'grid-list' && theme.density <= -5 ? [{ selector: '.grid-tile-label', mediaMaxWidth: '500px', top: '31px' }] : []),
        ...(family === 'grid-list' && theme.typographyScale > 1 ? [{ selector: '.grid-tile-label', mediaMaxWidth: '500px', top: '29px' }] : []),
        ...
        { selector: '.chip:active', background: mixHex(theme.surfaceContainer, theme.onSurface, .12) },
        { selector: '.chip.selected:hover', background: mixHex('#eadef7', '#4b4357', .08) },
        { selector: '.chip.selected:active', background: mixHex('#eadef7', '#4b4357', .12) },
        ...
        { selector: '.sort-header', position: 'relative', width: '100%', height: `${theme.density === -2 ? 22 : 19}px`, borderWidth: '0', background: 'transparent', color: theme.density <= -5 ? '#000000' : theme.onSurface, textAlign: 'left', fontWeight: '400' },
        ...
        { selector: '.tab', width: '50%', height: `${theme.density === 0 ? 48 : theme.density <= -5 ? 32 : 40}px`, boxSizing: 'border-box', padding: theme.density <= -5 ? '1px 0 0' : '0', flexShrink: '0', borderWidth: '0', borderRadius: '0', background: 'transparent', color: theme.onSurface, fontSize: '14px', fontWeight: '500', lineHeight: '20px', verticalAlign: 'middle' },
        { selector: '.tab:hover', background: mixHex(theme.surface, theme.onSurface, .08) },
        { selector: '.tab:focus', background: mixHex(theme.surfaceContainer, theme.onSurface, .12) },
        { selector: '.tab:active', background: mixHex(theme.surface, theme.onSurface, .12) },
        { selector: '.tab-baseline', position: 'absolute', zIndex: '20', top: `${theme.density === 0 ? 48 : theme.density <= -5 ? 32.5 : 39}px`, left: '0', width: '100%', height: '1px', background: '#e8e0eb' },
        { selector: '.tab-indicator', position: 'absolute', zIndex: '21', top: `${theme.density === 0 ? 46 : theme.density <= -5 ? 30.5 : 38}px`, left: state.selected ? '0' : '50%', width: '50.7%', height: '2px', background: theme.primary },
        ...
        { selector: '.tab-baseline', mediaMaxWidth: '500px', zIndex: '0' },
        { selector: '.tab-indicator', mediaMaxWidth: '500px', zIndex: '2' },
        { selector: '.tab-panel', width: '100%', height: '20px', boxSizing: 'border-box', verticalAlign: 'middle' },
        ...(family === 'tabs' && theme.typographyScale > 1 ? [{ selector: '.tab-panel', height: '22px' }] : []),
        ...
        { selector: '.step-tab', position: 'absolute', zIndex: '2', top: '0', width: '130px', height: `${theme.density === 0 ? 72 : theme.density <= -5 ? 48 : 64}px`, boxSizing: 'border-box', borderRadius: `${12 * theme.cornerScale}px`, cursor: 'pointer' },
        { selector: '.step-tab:hover', background: mixHex(theme.surface, theme.onSurface, theme.mode === 'dark' ? .01 : .08) },
        { selector: '.step-tab:focus', background: mixHex(theme.surface, theme.onSurface, theme.mode === 'dark' ? .015 : .08) },
        { selector: '.step-tab:active', background: mixHex(theme.surface, theme.onSurface, .12) },
        { selector: '#step-details', left: '-24px' },
        { selector: '#step-review', right: '-24px' },
        { selector: '.step-tab-content', position: 'absolute', top: `${theme.density === 0 ? 12 : theme.density <= -5 ? -3 : 7}px`, left: '24px', height: '48px', display: 'flex', alignItems: 'center' },
        ...
        { selector: '.step-badge.selected, .step-badge.completed', background: theme.primary, color: theme.onPrimary },
        { selector: '.step-complete-mark', width: '16px', height: '16px' },
        { selector: '.step-text', marginLeft: '8px', whiteSpace: 'nowrap', fontWeight: '500', fontSize: '14px', verticalAlign: 'middle' },
        { selector: '.step-connector', position: 'absolute', zIndex: '3', top: `${theme.density === 0 ? 35 : theme.density <= -5 ? 21 : 32}px`, left: '89px', width: '73.2%', height: '1px', background: '#7b757f', pointerEvents: 'none' },
        ...
        { selector: '.stepper-content-label', position: 'relative', top: `${theme.density <= -5 ? state.selected ? -2 : -3 : 0}px`, display: 'block', width: '100%', textAlign: 'left', verticalAlign: 'middle' },
        { selector: '.step-connector', mediaMaxWidth: '800px', width: '69.5%' },
        { selector: '.step-connector', mediaMaxWidth: '500px', left: '90px', width: '15.1%' },
        { selector: '.stepper-content-label', mediaMaxWidth: '500px', ...(theme.density <= -5 ? { top: state.selected ? '-1px' : '-2px' } : {}) },
        { selector: '.divider', position: 'absolute', top: `${theme.density === -2 ? 86.785 : theme.density <= -5 ? 74.785 : 79}px`, left: '28px', right: '28px', height: '1px', width: 'auto', background: '#cac4d0' },
        { selector: '.divider-copy', position: 'absolute', left: '28px', right: '28px', width: 'auto', height: `${22 * theme.typographyScale}px` },
        { selector: '.divider-above', top: `${theme.density === -2 ? 47.290625 - (devicePixelRatio >= 1.5 ? .5 : 0) : theme.density <= -5 ? 42.890625 - (devicePixelRatio >= 1.5 ? .5 : 0) : 43.69}px` },
        { selector: '.divider-below', top: `${theme.density === -2 ? 107.071875 : theme.density <= -5 ? 90.671875 : 95.69}px` },
        ...(family === 'divider' && theme.density <= -5 ? [
          { selector: '.divider-above', mediaMaxWidth: '500px', top: '42.390625px' },
          { selector: '.divider-below', mediaMaxWidth: '500px', top: '90.671875px' },
        ] : []),
        ...(family === 'divider' && theme.density === -2 ? [
          { selector: '.divider-above', mediaMaxWidth: '500px', top: '46.490625px' },
          { selector: '.divider-below', mediaMaxWidth: '500px', top: '106.271875px' },
        ] : []),
        ...
        { selector: '.dialog-title', width: '100%', height: '67px', boxSizing: 'border-box', margin: '0 0 1px', padding: '7px 24px 12px', display: 'flex', alignItems: 'flex-end', color: '#1d1b20', fontSize: '24px', lineHeight: '32px', fontWeight: '400' },
        { selector: '.dialog-copy', width: '100%', height: '20px', boxSizing: 'border-box', margin: '0', padding: '2px 24px 0', display: 'flex', alignItems: 'center', color: '#49454f', fontSize: '14px', lineHeight: '20px', verticalAlign: 'middle' },
        ...
        { selector: '.dialog-action:hover', background: mixHex('#fef8fc', '#7d00fa', .08) },
        { selector: '.dialog-action:active', background: mixHex('#fef8fc', '#7d00fa', .12) },
        { selector: '.dialog-action.primary:hover', background: mixHex('#7d00fa', '#ffffff', .08) },
        ...
        { selector: '.tooltip-anchor', width: '138px', height: '72px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', flexShrink: '0' },
        { selector: '.tooltip-anchor .material-button', flexShrink: '0' },
        { selector: '#tooltip-popup', position: 'relative', height: '24px', padding: '4px 8px', boxSizing: 'border-box', borderWidth: '0', borderRadius: '4px', background: '#323033', color: '#f5eff4', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: '0', fontSize: '12px', lineHeight: '16px', fontWeight: '400', letterSpacing: '0.4px', whiteSpace: 'nowrap', zIndex: '1000' },
      ],
    };
  }
  ...
  private familyElements(family: MaterialFamily): DOMElement[] {
    ...
    const fieldActiveLine = (controlId: string): DOMElement[] => this.focusedId() === controlId || state.error ? [{
        type: 'div',
        id: `${controlId}-active-line`,
        class: 'field-active-line',
      }] : [];
    ...
    if (family === 'sort') {
      ...
      const focusVisible = this.focusVisibleId() === 'sort-primary' || this.focusVisibleId() === 'sort-trigger';
      return [{ type: 'div', id: 'sort-primary', class: `sort-header${focused ? ' focused' : ''}${focusVisible ? ' focus-visible' : ''}`, ariaLabel: 'Sort by name', children: [{
    ...
    if (['input', 'form-field', 'autocomplete'].includes(family)) {
      ...
      ...fieldActiveLine(`${family}-control`),
    ...
    if (family === 'radio') return [{ type: 'div', id: 'radio-primary', role: 'radiogroup', children: [
      { type: 'div', id: 'radio-solo', class: 'radio-option', role: 'radio', tabindex: state.selected ? -1 : 0, ariaChecked: !state.selected, ariaDisabled: state.disabled, children: [{ type: 'span', id: 'radio-solo-state-layer', class: `radio-state-layer${state.selected ? '' : ' selected'}` }, { type: 'span', id: 'radio-solo-ring', class: `radio-ring${state.selected ? '' : ' selected'}`, children: state.selected ? [] : [{ type: 'span', id: 'radio-solo-dot', class: 'radio-dot' }] }, { type: 'span', id: 'radio-solo-label', class: 'radio-label', textContent: 'Solo' }] },
      { type: 'div', id: 'radio-team', class: 'radio-option', role: 'radio', tabindex: state.selected ? 0 : -1, ariaChecked: state.selected, ariaDisabled: state.disabled, children: [{ type: 'span', id: 'radio-team-state-layer', class: `radio-state-layer${state.selected ? ' selected' : ''}` }, { type: 'span', id: 'radio-team-ring', class: `radio-ring${state.selected ? ' selected' : ''}`, children: state.selected ? [{ type: 'span', id: 'radio-team-dot', class: 'radio-dot' }] : [] }, { type: 'span', id: 'radio-team-label', class: 'radio-label', textContent: 'Team' }] },
    ] }];
    if (family === 'select') {
      ...
      ...fieldActiveLine('select-control'),
    ...
    if (family === 'slide-toggle') return [{ type: 'div', id: 'slide-toggle-primary', role: 'switch', tabindex: state.disabled ? -1 : 0, ariaLabel: 'Automatic updates', ariaChecked: state.selected, ariaDisabled: state.disabled, children: [
      ...
      { type: 'span', id: 'slide-toggle-track', class: 'switch-track', children: [{ type: 'span', id: 'slide-toggle-thumb', class: 'switch-thumb', children: state.selected ? [this.selectionMark('slide-toggle-mark', 'switch-mark')] : [{ type: 'span', id: 'slide-toggle-minus', class: 'switch-minus' }] }] },
      { type: 'span', id: 'slide-toggle-label', class: 'switch-label', textContent: 'Automatic updates' },
    ] }];
    if (family === 'menu') return [{ type: 'button', id: 'menu-primary', class: 'material-button', ariaHaspopup: 'menu', ariaExpanded: state.open, ariaControls: 'menu-popup', value: 'Open menu' }, ...(state.open ? [{ type: 'div' as const, id: 'menu-popup', role: 'menu', children: [
      { type: 'button' as const, id: 'menu-rename', role: 'menuitem', ariaLabel: 'Rename', children: [{ type: 'span' as const, id: 'menu-rename-label', class: 'menu-option-label', textContent: 'Rename' }] },
      { type: 'button' as const, id: 'menu-delete', role: 'menuitem', ariaLabel: 'Delete', children: [{ type: 'span' as const, id: 'menu-delete-label', class: 'menu-option-label', textContent: 'Delete' }] },
    ] }] : [])];
    if (family === 'tabs') return [{ type: 'div', id: 'tabs-primary', class: 'tabs', ariaLabel: `OverviewActivity${state.selected ? 'Overview content' : 'Activity content'}`, children: [{ type: 'div', id: 'tabs-list', class: 'tab-list', role: 'tablist', children: [{ type: 'button', id: 'tab-overview', class: 'tab', role: 'tab', ariaSelected: state.selected, tabindex: state.selected ? 0 : -1, ariaControls: 'tab-panel', value: 'Overview' }, { type: 'button', id: 'tab-activity', class: 'tab', role: 'tab', ariaSelected: !state.selected, tabindex: state.selected ? -1 : 0, ariaControls: 'tab-panel', value: 'Activity' }] }, { type: 'div', id: 'tab-baseline', class: 'tab-baseline' }, { type: 'div', id: 'tab-indicator', class: 'tab-indicator' }, { type: 'showcase.material:tab-panel', id: 'tab-panel', class: 'tab-panel', role: 'tabpanel', ariaLabel: state.selected ? 'Overview content' : 'Activity content', data: { selected: state.selected, phase: this.benchmarkMode ? 1 : undefined, 'text-color': theme.onSurface, 'font-size': 16 * theme.typographyScale, 'baseline-offset': theme.typographyScale > 1 ? -.2 : 0 } }] }];
    if (family === 'stepper') return [{ type: 'div', id: 'stepper-primary', class: 'stepper', role: 'tablist', ariaLabel: 'Project setup', children: [{ type: 'div', id: 'stepper-head', class: 'stepper-head', children: [{ type: 'div', id: 'step-details', class: 'step-tab', role: 'tab', tabindex: state.selected ? 0 : -1, ariaSelected: state.selected, children: [{ type: 'div', class: 'step-tab-content', children: [{ type: 'span', id: 'step-details-badge', class: `step-badge${state.selected ? ' selected' : ' completed'}`, children: state.selected ? undefined : [this.selectionMark('step-details-complete', 'step-complete-mark')], textContent: state.selected ? '1' : undefined }, { type: 'span', id: 'step-details-text', class: 'step-text', textContent: 'Details' }] }] }, { type: 'span', id: 'step-connector', class: 'step-connector' }, { type: 'div', id: 'step-review', class: 'step-tab', role: 'tab', tabindex: state.selected ? -1 : 0, ariaSelected: !state.selected, children: [{ type: 'div', class: 'step-tab-content', children: [{ type: 'span', id: 'step-review-badge', class: `step-badge${state.selected ? '' : ' selected'}`, textContent: '2' }, { type: 'span', id: 'step-review-text', class: 'step-text', textContent: 'Review' }] }] }] }, { type: 'div', id: 'stepper-content', role: 'tabpanel', children: [{ type: 'span', class: 'stepper-content-label', textContent: state.selected ? 'Project details' : 'Review changes' }] }] }];
    ...
    if (family === 'expansion') return [{ type: 'article', id: 'expansion-shell', class: 'expansion-panel', children: [{ type: 'div', id: 'expansion-primary', class: `expansion-trigger${state.disabled ? ' disabled' : ''}`, role: 'button', tabindex: state.disabled ? -1 : 0, ariaDisabled: state.disabled, ariaExpanded: state.open, ariaControls: 'expansion-content', children: [{ type: 'span', id: 'expansion-title', class: 'expansion-title', textContent: 'Advanced settings' }] }, { type: 'span', id: 'expansion-chevron', class: `expansion-chevron${state.disabled ? ' disabled' : ''}` }, { type: 'p' as const, id: 'expansion-content', children: [{ type: 'span' as const, id: 'expansion-content-label', class: 'expansion-content-label', textContent: 'Additional options.' }] }] }];
    ...
    if (family === 'tooltip') return [{ type: 'div', id: 'tooltip-anchor', class: 'tooltip-anchor', children: [
      { type: 'button', id: 'tooltip-primary', class: 'material-button', value: 'Hover for help', ariaDescribedby: state.open ? 'tooltip-popup' : undefined },
      ...(state.open ? [{ type: 'div' as const, id: 'tooltip-popup', role: 'tooltip', textContent: 'Create a project' }] : []),
    ] }];
    if (family === 'core') {
  ...
  private selectionMark(id: string, extraClass = ''): DOMElement {
    const theme = this.store.tokens();
    const color = extraClass === 'checkbox-mark' || extraClass === 'step-complete-mark'
      ? theme.onPrimary
      : extraClass === 'switch-mark' ? theme.primary : '#49454f';
    return {
...
function sliderHandleName(elementId: string | undefined): '' | 'start' | 'end' {
  if (elementId === 'slider-start') return 'start';
  if (elementId === 'slider-primary') return 'end';
  return '';
}
