...
function focusedProofInventory(root) {
  return [
    ...
    proof(root, 'examples/material-showcase/src/app/input-equivalence-proof.spec.ts', /describe\('Material audit/,
      'seventeen executable browser reductions; eight pass and nine diagnostic failures are retained', 'Eight original reductions pass: toolbar, stepper, content-derived flex height, calendar span, bottom overlay, table cells, inherited text stage, and positioned drawer geometry. Nine failures remain: the divider empty-block height defect; two original-calc grid-list cases exercising the documented direct-style expression limitation; two literal grid-list controls whose outer geometry/gutter match but content height fails; two plain block/flex opposing-inset height reductions; and two original-calc cases through core loaded document styles. The loaded-CSS cases resolve width/height/left and outer boxes correctly, but expose the same inner opposing-inset auto-height failure. Retained text is not a full computed-style/current-paint guarantee. Passing geometry does not prove text/border raster, scrolling or full Material composition. Consult the investigation for exact commands and limits.'),
