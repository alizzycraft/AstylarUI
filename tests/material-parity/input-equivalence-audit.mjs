...
function focusedProofInventory(root) {
  return [
    proof(root, 'examples/material-showcase/src/app/grid-template-initial-audit.spec.ts', /describe\('Material audit: grid initial template/,
      'confirmed failing none templates; omitted and explicit-track controls pass', 'Two real-browser runs each retain four equal-input none failures (columns/rows at120/240px) and twelve passing omitted,1fr and literal controls. Authored inputs, style stages, parent geometry, assigned CSS item sizes, projected edges and zero-resource cleanup isolate the track-sizing boundary. This DPR1 geometry proof does not establish final raster or broader implicit-grid semantics.'),
