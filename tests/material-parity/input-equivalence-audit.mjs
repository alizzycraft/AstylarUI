...
function implementationPlan() {
  return [
    ...
    { priority: 5.1, rootCause: 'Core rewrites an explicit font-family list before paint', action: 'The parser appends Arial, Helvetica, sans-serif to explicit lists without a recognized generic. Preserve this as distinct from missing Material font-token authoring. The equal-input Arial reduction proves the input mutation but not glyph impact; add an unavailable-font or missing-glyph browser proof, then preserve authored family ordering/quoting and browser fallback semantics at the core parser boundary. Do not add a generic family to the showcase merely to avoid the parser branch.' },
