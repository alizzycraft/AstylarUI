...
import { buttonFlexAttribution, collectButtonFlexInputs, classifyButtonFlexInput,
  validateButtonFlexInputs, validateButtonFlexClassifications } from './button-flex-source-binding.mjs';
import { buttonHostRequestAttribution, collectButtonHostRequestInputs, classifyButtonHostRequestInput,
...
import { buttonFixedWidthAttribution, classifyButtonFixedWidthInput,
  validateButtonFixedWidthClassifications } from './button-fixed-width-classification.mjs';
import { collectOwnerGridInitialInputs, validateOwnerGridInitialInputs } from './owner-grid-initial-source-binding.mjs';
import { ownerGridInitialAttribution, classifyOwnerGridInitialInput } from './owner-grid-initial-classification.mjs';
import { validateOwnerGridInitialClassifications } from './owner-grid-initial-coverage.mjs';
import { collectButtonBoxSizingInputs, validateButtonBoxSizingInputs } from './button-box-sizing-source-binding.mjs';
import { buttonBoxSizingAttribution, classifyButtonBoxSizingInput } from './button-box-sizing-classification.mjs';
...
import { collectExplicitGapInputs, validateExplicitGapInputs, explicitGapClassificationContexts } from './explicit-gap-source-binding.mjs';
...
import { collectGapReviewInputs, validateGapReviewInputs, gapReviewClassificationContexts } from './gap-review-source-binding.mjs';
...
import { collectOwnerCaretAuditInputs, validateOwnerCaretAuditInputs } from './owner-caret-audit-source-binding.mjs';
import { collectReviewedInputAuditInputs, validateReviewedInputAuditInputs, reviewedInputClassificationContexts,
  classifyReviewedInput, validateReviewedInputClassifications } from './reviewed-input-audit-source-binding.mjs';
...
import { collectReviewedSourceBatchAuditInputs, validateReviewedSourceBatchAuditInputs, reviewedSourceBatchClassificationContexts,
...
import { ownerInitialStyleAttribution, bindOwnerInitialStyleSource, collectOwnerInitialStyleEvidence,
  classifyOwnerInitialStyleInput, validateOwnerInitialStyleSource, readOwnerInitialStyleSource } from './owner-initial-style-attribution.mjs';
...
import { rootColorAttribution, collectRootColorInputs, classifyRootColorInput,
  fieldColorAttribution, collectFieldColorInputs, classifyFieldColorInput,
  containerCaretAttribution, collectContainerCaretInputs, classifyContainerCaretInput } from './root-color-input-evidence.mjs';
...
export function buildMaterialInputAudit(parityReport, options = {}) {
  ...
  const ownerInitialStyleBinding = bindOwnerInitialStyleSource(parityReport, { root, parityPath: options.parityPath });
  const ownerInitialStyleEvidence = collectOwnerInitialStyleEvidence(
    ownerInitialStyleBinding.status === 'bound' ? parityReport : {}, elementInventory);
  ...
  const buttonFlexInputs = collectButtonFlexInputs(parityReport, { root, parityPath: options.parityPath });
  const buttonHostRequestInputs = collectButtonHostRequestInputs(parityReport, { root, parityPath: options.parityPath });
  const buttonFixedWidthInputs = collectButtonFixedWidthInputs(parityReport, { root, parityPath: options.parityPath });
  const ownerGridInitialInputs = collectOwnerGridInitialInputs(parityReport, { root, parityPath: options.parityPath });
  const buttonBoxSizingInputs = collectButtonBoxSizingInputs(parityReport, { root, parityPath: options.parityPath });
  const fieldHostLayoutInputs = collectBoundFieldHostLayout(parityReport, { root, parityPath: options.parityPath, collectInventory: collectFullTreeInventory });
  const ownerGapInputs = collectOwnerGapInputs(parityReport, { root, parityPath: options.parityPath });
  const explicitGapInputs = collectExplicitGapInputs(parityReport, { root, parityPath: options.parityPath });
  const gapReviewInputs = collectGapReviewInputs(parityReport, { root, parityPath: options.parityPath });
  ...
  const reviewedSourceBatchInputs = collectReviewedSourceBatchAuditInputs(parityReport, { root, parityPath: options.parityPath });
  const rootBackgroundInputs = collectRootBackgroundAuditInputs(parityReport, { root, parityPath: options.parityPath });
  ...
  const discrepancies = ownerInitialStyleBinding.status === 'bound'
    ? applyBottomSheetScalarTypography(applyDialogScalarTypography(applyDialogActionBox(overlaySurfaceDiscrepancies, cases, elementInventory, canonicalStyle), cases, elementInventory, retainedTypography, controlTypography, canonicalStyle), cases, elementInventory, canonicalStyle)
  ...
  const report = {
    ...
    generatedFrom: {
      ...
      captureProvenance: parityReport.captureProvenance,
    },
    contract: {
    ...
    summary: {
      inputEquivalent: coverage.complete && coverage.missingElements.length === 0 &&
        buttonFixedWidthInputs.binding.status === 'bound' && buttonFixedWidthInputs.groups.length === 0 &&
    ...
    buttonFlexInputs,
    buttonHostRequestInputs,
    buttonFixedWidthInputs,
    ownerGridInitialInputs,
    buttonBoxSizingInputs,
    fieldHostLayoutInputs,
    ownerGapInputs,
    explicitGapInputs,
    gapReviewInputs,
    ownerCaretInputs,
    ...
    ownerInitialStyleBinding,
    ownerInitialStyleEvidence,
...
export function validateMaterialInputAudit(report, { requireComplete = true, root = process.cwd() } = {}) {
  ...
  if (report.followupInputs?.binding?.status === 'bound') {
    ...
    errors.push(...sourceErrors);
    if (!sourceErrors.length) errors.push(...validateFollowupInputClassifications(report.followupInputs, report.discrepancies));
    if (report.followupInputs.observations.length && report.summary?.inputEquivalent)
      errors.push('followup input differences do not establish input equivalence');
  } else if (requireComplete || report.followupInputs?.binding?.status === 'invalid' ||
      report.followupInputs?.observations?.length || report.discrepancies?.some(d => followupInputAttributions.includes(d.attribution))) {
    errors.push('followup input attributions lack independently bound original source evidence');
  }
  if (report.reviewedInputs?.binding?.status === 'bound') {
    const sourceErrors = validateReviewedInputAuditInputs(report.reviewedInputs, { root, requireComplete });
    errors.push(...sourceErrors);
    if (!sourceErrors.length) errors.push(...validateReviewedInputClassifications(report.reviewedInputs, report.discrepancies));
    if (report.reviewedInputs.observations.length && report.summary?.inputEquivalent)
      errors.push('reviewed input differences do not establish input equivalence');
  } else if (requireComplete || report.reviewedInputs?.binding?.status === 'invalid' ||
      report.reviewedInputs?.observations?.length || report.discrepancies?.some(d => reviewedInputAttributions.includes(d.attribution))) {
    errors.push('reviewed input attributions lack independently bound original source evidence');
  }
  if (report.ownerCaretInputs?.binding?.status === 'bound') {
    const sourceErrors = validateOwnerCaretAuditInputs(report.ownerCaretInputs, { root, requireComplete });
    errors.push(...sourceErrors);
    if (!sourceErrors.length)
      errors.push(...validateOwnerCaretAttributionRows(report.ownerCaretInputs.plannedCoverage, report.discrepancies));
    if (report.ownerCaretInputs.observations?.length && report.summary?.inputEquivalent)
      errors.push('caret observation-stage findings do not establish input equivalence');
  } else if (requireComplete || report.ownerCaretInputs?.binding?.status === 'invalid' ||
      report.ownerCaretInputs?.observations?.length || report.discrepancies?.some(d => Object.values(ownerCaretAttributions).includes(d.attribution))) {
    errors.push('caret attribution lacks independently bound original capture evidence');
  }
  if (report.gapReviewInputs?.binding?.status === 'bound') {
    errors.push(...validateGapReviewInputs(report.gapReviewInputs, { root, requireComplete }));
    errors.push(...validateGapReviewClassifications(report.gapReviewInputs, report.discrepancies, { root, requireComplete }));
    if (report.gapReviewInputs.observations.length && report.summary?.inputEquivalent)
      errors.push('gap review capture and observation-stage findings do not establish input equivalence');
  } else if (requireComplete || report.gapReviewInputs?.binding?.status === 'invalid' ||
      report.gapReviewInputs?.observations?.length || report.discrepancies?.some(d => Object.values(gapReviewAttributions).includes(d.attribution))) {
    errors.push('gap review attribution lacks independently bound original capture evidence');
  }
  if (report.explicitGapInputs?.binding?.status === 'bound') {
    errors.push(...validateExplicitGapInputs(report.explicitGapInputs, { root, requireComplete }));
    errors.push(...validateExplicitGapClassifications(report.explicitGapInputs, report.discrepancies, { root, requireComplete }));
    if (report.explicitGapInputs.observations.length && report.summary?.inputEquivalent)
      errors.push('explicit gap authoring differences do not establish input equivalence');
  } else if (requireComplete || report.explicitGapInputs?.binding?.status === 'invalid' ||
      report.explicitGapInputs?.observations?.length || report.discrepancies?.some(d => d.attribution === explicitGapAttribution)) {
    errors.push('explicit gap attribution lacks independently bound original capture evidence');
  }
  if (report.ownerGapInputs?.binding?.status === 'bound') {
    errors.push(...validateOwnerGapInputs(report.ownerGapInputs, { root }));
    errors.push(...validateOwnerGapClassifications(report.ownerGapInputs, report.discrepancies, { root }));
    if (report.ownerGapInputs.observations.length && report.summary?.inputEquivalent)
      errors.push('owner gap observation-stage attribution does not establish input equivalence');
  } else if (requireComplete || report.ownerGapInputs?.binding?.status === 'invalid' ||
      report.ownerGapInputs?.observations?.length || report.discrepancies?.some(d => d.attribution === ownerGapAttribution)) {
    errors.push('owner gap attribution lacks independently bound original capture evidence');
  }
  if (report.fieldHostLayoutInputs?.binding?.status === 'bound') {
    errors.push(...validateBoundFieldHostLayout(report.fieldHostLayoutInputs, { root, collectInventory: collectFullTreeInventory }));
    errors.push(...validateFieldHostLayoutClassifications(report.fieldHostLayoutInputs, report.discrepancies, canonicalStyle, { root }));
    if (report.fieldHostLayoutInputs.observations.length && report.summary?.inputEquivalent)
      errors.push('field-host layout authoring and observation-stage attribution does not establish input equivalence');
  } else if (requireComplete || report.fieldHostLayoutInputs?.binding?.status === 'invalid' ||
      report.fieldHostLayoutInputs?.observations?.length || report.discrepancies?.some(d => [fieldHostLayoutAttribution, fieldHostWidthAttribution].includes(d.attribution))) {
    errors.push('field-host layout attribution lacks independently bound original capture evidence');
  }
  if (report.buttonBoxSizingInputs?.binding?.status === 'bound') {
    errors.push(...validateButtonBoxSizingInputs(report.buttonBoxSizingInputs, { root }));
    errors.push(...validateButtonBoxSizingClassifications(report.buttonBoxSizingInputs, report.discrepancies, { root }));
    if (report.buttonBoxSizingInputs.groups.length && report.summary?.inputEquivalent)
      errors.push('button box sizing observation-stage attribution does not establish input equivalence');
  } else if (requireComplete || report.buttonBoxSizingInputs?.binding?.status === 'invalid' ||
      report.buttonBoxSizingInputs?.observations?.length || report.discrepancies?.some(d => d.attribution === buttonBoxSizingAttribution)) {
    errors.push('button box sizing attribution lacks independently bound original capture evidence');
  }
  if (report.ownerGridInitialInputs?.binding?.status === 'bound') {
    errors.push(...validateOwnerGridInitialInputs(report.ownerGridInitialInputs, { root }));
    errors.push(...validateOwnerGridInitialClassifications(report.ownerGridInitialInputs,
      report.discrepancies, report.nonGridTemplateInputs, { root }));
    if (report.summary?.inputEquivalent && report.discrepancies.some(d => d.attribution === ownerGridInitialAttribution))
      errors.push('owner grid observation-stage attribution does not establish input equivalence');
  } else if (requireComplete || report.ownerGridInitialInputs?.binding?.status === 'invalid' ||
      report.ownerGridInitialInputs?.observations?.length || report.discrepancies?.some(d => d.attribution === ownerGridInitialAttribution)) {
    errors.push('owner grid attribution lacks independently bound original capture evidence');
  }
  if (report.buttonFixedWidthInputs?.binding?.status === 'bound') {
    errors.push(...validateButtonFixedWidthInputs(report.buttonFixedWidthInputs, { root }));
    errors.push(...validateButtonFixedWidthClassifications(report.buttonFixedWidthInputs,
      report.discrepancies, canonicalStyle, equivalentValue, { root }));
    if (report.buttonFixedWidthInputs.groups.length && report.summary?.inputEquivalent)
      errors.push('button fixed width authoring differences contradict claimed input equivalence');
  } else if (requireComplete || report.buttonFixedWidthInputs?.binding?.status === 'invalid' ||
      report.buttonFixedWidthInputs?.observations?.length || report.discrepancies?.some(d => d.attribution === buttonFixedWidthAttribution)) {
    errors.push('button fixed width attribution lacks independently bound original capture evidence');
  }
  if (report.buttonFlexInputs?.binding?.status === 'bound') {
    errors.push(...validateButtonFlexInputs(report.buttonFlexInputs, { root }));
    errors.push(...validateButtonFlexClassifications(report.buttonFlexInputs,
      report.discrepancies, canonicalStyle, equivalentValue));
  } else if (requireComplete || report.buttonFlexInputs?.binding?.status === 'invalid' ||
      report.buttonFlexInputs?.observations?.length || report.discrepancies?.some(d => d.attribution === buttonFlexAttribution)) {
    errors.push('button flex attribution lacks independently bound original capture evidence');
  }
  if (report.buttonHostRequestInputs?.binding?.status === 'bound') {
    errors.push(...validateButtonHostRequestInputs(report.buttonHostRequestInputs, { root }));
    errors.push(...validateButtonHostRequestClassifications(report.buttonHostRequestInputs,
      report.discrepancies, canonicalStyle, equivalentValue));
  } else if (requireComplete || report.buttonHostRequestInputs?.binding?.status === 'invalid' ||
      report.buttonHostRequestInputs?.observations?.length || report.discrepancies?.some(d => d.attribution === buttonHostRequestAttribution)) {
    errors.push('button host request attribution lacks independently bound original capture evidence');
  }
  ...
  if (report.rootShadowInputs?.binding?.status === 'bound') {
    ...
    errors.push(...validateRootShadowClassifications(report.rootShadowInputs,
      report.discrepancies, canonicalStyle, equivalentValue));
  } else if (requireComplete || report.rootShadowInputs?.binding?.status === 'invalid' ||
      report.rootShadowInputs?.observations?.length || report.discrepancies?.some(d => d.attribution === rootShadowAttribution)) {
    errors.push('root shadow attribution lacks independently bound original capture evidence');
  }
  ...
  if (report.ownerInitialStyleBinding?.status === 'bound') {
    ...
    try {
      const original = readOwnerInitialStyleSource(report.ownerInitialStyleBinding, { root });
      const replayedEvidence = collectOwnerInitialStyleEvidence(original, report.elementInventory);
      if (JSON.stringify(replayedEvidence) !== JSON.stringify(report.ownerInitialStyleEvidence))
        errors.push('owner initial-style evidence differs from complete captured inventory replay');
      const cases = [...(original.results ?? []).map(e => ({ ...e, kind: 'static' })),
        ...(original.interactions ?? []).map(e => ({ ...e, kind: 'interaction' }))];
      const replayedRows = collectStyleDiscrepancies(cases, report.originStageEvidence, report.retainedTypography,
        report.visibleOverflowInputs, report.borderInitialInputs, report.buttonBorderResetInputs,
        report.outlineTokenInputs, report.chipOutlineInputs, report.nonGridTemplateInputs,
        report.buttonTypographyScalarInputs, report.chipHostTypographyInputs, report.fieldHostTypographyInputs,
        report.rootTypographyInputs, report.appearanceInitialInputs, report.buttonAppearanceInputs,
      ...
      errors.push(...validateDialogActionBox(report.discrepancies, replayedRows, cases,
        report.elementInventory, canonicalStyle));
      ...
      report.discrepancies?.some(d => [ownerInitialStyleAttribution, 'reviewed-dialog-scalar-typography-owner', 'reviewed-bottom-sheet-scalar-typography-owner', 'reviewed-dialog-action-box-substitution'].includes(d.attribution))) {
  ...
  if (report.sliderBorderDefaults?.binding?.status === 'bound') {
    ...
    errors.push(...validateSliderBorderDefaultClassifications(report.sliderBorderDefaults,
      report.discrepancies, canonicalStyle, equivalentValue));
  } else if (requireComplete || report.sliderBorderDefaults?.binding?.status === 'invalid' ||
      report.sliderBorderDefaults?.observations?.length || report.discrepancies?.some(d => d.attribution === sliderBorderDefaultAttribution)) {
    errors.push('slider border attribution lacks independently bound original capture evidence');
  }
  ...
  if (report.sliderInputBoxes?.binding?.status === 'bound') {
    errors.push(...validateSliderInputBoxes(report.sliderInputBoxes, { root }));
    errors.push(...validateSliderInputBoxClassifications(report.sliderInputBoxes,
      report.discrepancies, canonicalStyle, equivalentValue));
  } else if (requireComplete || report.sliderInputBoxes?.binding?.status === 'invalid' ||
      report.sliderInputBoxes?.observations?.length || report.discrepancies?.some(d => d.attribution === sliderInputBoxAttribution)) {
  ...
  for (const entry of report.discrepancies.filter(d => d.attribution === appearanceInitialAttribution)) {
    const proof = report.appearanceInitialInputs?.find(p => p.case === entry.reviewEvidence?.case && p.element === entry.element);
    if (!proof || entry.classification !== 'equivalent-representation' || entry.property !== 'appearance' || entry.reference !== 'none' || entry.astylar !== undefined ||
        JSON.stringify(proof) !== JSON.stringify(entry.reviewEvidence) || !Array.isArray(entry.reviewedCases) ||
        entry.reviewedCases.length !== entry.occurrences || new Set(entry.reviewedCases).size !== entry.occurrences ||
        entry.reviewedCases.some(key => !report.appearanceInitialInputs?.some(p => p.case === key && p.element === entry.element))) {
      errors.push('non-widget appearance classification lacks exact owner, stage and case evidence');
    }
  }
  ...
  if (JSON.stringify(report.rootColorInputs) !== JSON.stringify(collectRootColorInputs(
    collectRootTypographyInputs(report.elementInventory, canonicalStyle, typographySelectorCanApply), canonicalStyle))) {
    errors.push('root color evidence does not replay from captured ancestry and color declarations');
  }
  for (const entry of report.discrepancies.filter(d => d.attribution === rootColorAttribution)) {
    const proof = report.rootColorInputs?.find(p => p.case === entry.reviewEvidence?.case && p.element === entry.element);
    if (!proof || entry.classification !== 'parity-harness-defect' || entry.property !== 'color' || entry.reference !== proof.values.reference || entry.astylar !== undefined ||
        JSON.stringify(proof) !== JSON.stringify(entry.reviewEvidence) || !Array.isArray(entry.reviewedCases) ||
        entry.reviewedCases.length !== entry.occurrences || new Set(entry.reviewedCases).size !== entry.occurrences ||
        entry.reviewedCases.some(key => !report.rootColorInputs?.some(p => p.case === key && p.element === entry.element && p.values.reference === entry.reference))) {
      errors.push('root color classification lacks exact ancestry, stage and case evidence');
    }
  }
  if (JSON.stringify(report.fieldColorInputs) !== JSON.stringify(collectFieldColorInputs(
    collectFieldHostTypographyInputs(report.elementInventory, canonicalStyle, typographySelectorCanApply),
    collectRootColorInputs(collectRootTypographyInputs(report.elementInventory, canonicalStyle, typographySelectorCanApply), canonicalStyle), canonicalStyle))) {
    errors.push('field host color evidence does not replay from independent host identity and root ancestry');
  }
  for (const entry of report.discrepancies.filter(d => d.attribution === fieldColorAttribution)) {
    const proof = report.fieldColorInputs?.find(p => p.case === entry.reviewEvidence?.case && p.element === entry.element);
    if (!proof || entry.classification !== 'parity-harness-defect' || entry.property !== 'color' || entry.reference !== proof.values.reference || entry.astylar !== undefined ||
        JSON.stringify(proof) !== JSON.stringify(entry.reviewEvidence) || !Array.isArray(entry.reviewedCases) ||
        entry.reviewedCases.length !== entry.occurrences || new Set(entry.reviewedCases).size !== entry.occurrences ||
        entry.reviewedCases.some(key => !report.fieldColorInputs?.some(p => p.case === key && p.element === entry.element && p.values.reference === entry.reference))) {
      errors.push('field host color classification lacks exact ancestry, stage and case evidence');
    }
  }
  const caretRoots = collectRootColorInputs(collectRootTypographyInputs(report.elementInventory, canonicalStyle, typographySelectorCanApply), canonicalStyle);
  const caretFields = collectFieldColorInputs(collectFieldHostTypographyInputs(report.elementInventory, canonicalStyle, typographySelectorCanApply), caretRoots, canonicalStyle);
  if (JSON.stringify(report.containerCaretInputs) !== JSON.stringify(collectContainerCaretInputs([...caretRoots, ...caretFields], canonicalStyle))) {
    errors.push('container caret evidence does not replay from complete color and caret declaration ancestry');
  }
  for (const entry of report.discrepancies.filter(d => d.attribution === containerCaretAttribution)) {
    const proof = report.containerCaretInputs?.find(p => p.case === entry.reviewEvidence?.case && p.element === entry.element);
    if (!proof || entry.classification !== 'parity-harness-defect' || entry.property !== 'caretColor' ||
        entry.reference !== proof.values.reference || entry.astylar !== undefined ||
        JSON.stringify(proof) !== JSON.stringify(entry.reviewEvidence) || !Array.isArray(entry.reviewedCases) ||
        entry.reviewedCases.length !== entry.occurrences || new Set(entry.reviewedCases).size !== entry.occurrences ||
        entry.reviewedCases.some(key => !report.containerCaretInputs?.some(p => p.case === key && p.element === entry.element && p.values.reference === entry.reference))) {
      errors.push('container caret attribution lacks exact container, declaration, stage and case evidence');
    }
  }
  ...
  if (JSON.stringify(report.rootHeightInputs) !== JSON.stringify(collectRootHeightInputs(
    collectRootTypographyInputs(report.elementInventory, canonicalStyle, typographySelectorCanApply), canonicalStyle))) {
    errors.push('root height evidence does not replay from captured declaration constraints');
  }
  for (const entry of report.discrepancies.filter(d => d.attribution === rootHeightAttribution)) {
    const proof = report.rootHeightInputs?.find(p => p.case === entry.reviewEvidence?.case && p.element === entry.element);
    if (!proof || entry.classification !== 'application-plugin-authoring-defect' || entry.property !== 'height' ||
        entry.reference !== proof.values.referenceUsedHeight || entry.astylar !== canonicalStyle({ height: proof.values.candidateHeightDeclaration }).height ||
        JSON.stringify(proof) !== JSON.stringify(entry.reviewEvidence) || !Array.isArray(entry.reviewedCases) ||
        entry.reviewedCases.length !== entry.occurrences || new Set(entry.reviewedCases).size !== entry.occurrences ||
  ...
  if (JSON.stringify(report.fieldHostAlignmentInputs) !== JSON.stringify(collectFieldHostAlignmentInputs(
    collectFieldHostTypographyInputs(report.elementInventory, canonicalStyle, typographySelectorCanApply)))) {
    errors.push('field host alignment evidence does not replay from captured host and alignment requests');
  }
  for (const entry of report.discrepancies.filter(d => d.attribution === fieldHostAlignmentAttribution)) {
    const proof = report.fieldHostAlignmentInputs?.find(p => p.case === entry.reviewEvidence?.case && p.element === entry.element);
    if (!proof || entry.classification !== 'application-plugin-authoring-defect' || entry.property !== 'textAlign' ||
        entry.reference !== 'left' || entry.astylar !== undefined || JSON.stringify(proof) !== JSON.stringify(entry.reviewEvidence) ||
        ...
        entry.reviewedCases.some(key => !report.fieldHostAlignmentInputs?.some(p => p.case === key && p.element === entry.element))) {
      errors.push('field host alignment classification lacks exact declaration, stage and case evidence');
    }
  }
  ...
  for (const entry of report.discrepancies.filter(d => d.attribution === fieldHostTypographyAttribution)) {
    const proof = report.fieldHostTypographyInputs?.find(p => p.case === entry.reviewEvidence?.case && p.element === entry.element && p.property === entry.property);
    if (!proof || entry.classification !== 'application-plugin-authoring-defect' || entry.reference !== proof.values.reference || entry.astylar !== undefined ||
        JSON.stringify(proof) !== JSON.stringify(entry.reviewEvidence) || !Array.isArray(entry.reviewedCases) ||
        entry.reviewedCases.length !== entry.occurrences || new Set(entry.reviewedCases).size !== entry.occurrences ||
        entry.reviewedCases.some(key => !report.fieldHostTypographyInputs?.some(p => p.case === key && p.element === entry.element && p.property === entry.property && p.values.reference === entry.reference))) {
      errors.push('field host typography classification lacks exact owner, stage and case evidence');
    }
  }
  ...
  const isCalendarCloseStateGap = gap => gap.attribution === 'reviewed-calendar-close-state-divergence' &&
    gap.classification === 'application-plugin-authoring-defect';
  const closeStateGaps = report.controlTypography?.gaps.filter(isCalendarCloseStateGap) ?? [];
  const replayedCloseStateGaps = collectControlTypographyEvidence(report.supplementalCalendarClose?.cases ?? [], report.elementInventory)
    .gaps.filter(isCalendarCloseStateGap);
  if (JSON.stringify(closeStateGaps) !== JSON.stringify(replayedCloseStateGaps)) errors.push('calendar close state-divergence controls lack complete replayed ownership');
  const unresolvedControlGaps = report.controlTypography?.gaps.filter(gap =>
    !isReviewedCalendarCloseGap(gap, report.elementInventory) && !isCalendarCloseStateGap(gap)) ?? [];
  ...
  for (const [family, attribution, label] of [
    ...
    const invalidTypography = report.controlTypography?.differences.filter(difference => {
      ...
      if (!replays.has(difference.case)) {
        ...
        const match = parseReviewedCase(difference.case, family);
  ...
  validatePaginatorTooltipOmissions(report, errors);
  ...
  const unresolvedRetainedGaps = retainedGaps.filter((gap) => !isReviewedHiddenRetainedGap(gap, report.elementInventory) &&
    ...
    !isReviewedPaginatorTooltipGap(gap, report.elementInventory) &&
...
export function validateCalendarCloseInventory(report, errors, { root = process.cwd(), requireComplete = true, readBytes } = {}) {
  const recorded = report.supplementalCalendarClose;
  if (!recorded || typeof recorded.file !== 'string') {
    errors.push('missing calendar close supplemental evidence');
    return;
  }
  const expected = collectCalendarCloseEvidence(root, { reportPath: recorded.file,
    expectedProvenance: report.generatedFrom?.captureProvenance, readBytes });
  if (JSON.stringify(recorded) !== JSON.stringify(expected)) errors.push('calendar close summary does not replay from bound source evidence');
  if (expected.errors.length) errors.push('calendar close supplemental collection errors');
  if (requireComplete && !expected.complete) errors.push('calendar close supplemental action coverage is incomplete or unbound');
  if (!report.elementInventory?.cases || !report.elementInventory?.variants || !report.elementInventory?.styles || !report.elementInventory?.rules) {
    errors.push('missing calendar close action-boundary inventory');
    return;
  }
  // Build a separate inventory from verified source trees. Interned indices in
  // the main inventory are not proof that their referenced values are correct.
  ...
  const snapshot = inventory => {
    ...
    return inventory.cases.filter(item => item.case.includes('/calendar-close-')).map(item => {
...
export function renderMaterialInputAuditMarkdown(report) {
  const lines = [
    ...
    '',
    `Reviewed source batch: ${report.reviewedSourceBatchInputs?.observations.length ?? 0} original observations support ${report.discrepancies.filter(d => reviewedSourceBatchAttributions.includes(d.attribution)).length} source-replayed motion-observation, layout-authoring and button-paint groups. Historical membership and receipts remain explicit; live classifications use precise current scalar values. Prior classifications are preserved. This does not establish input equivalence, rendering parity, inactive motion or renderer causality.`,
    `Root background authoring: ${report.rootBackgroundInputs?.observations.length ?? 0} original observations support ${report.discrepancies.filter(d => d.attribution === rootBackgroundAttribution).length} source-replayed fractional CSS mix versus integer-hex theme groups. Unequal inputs remain unequal; this does not establish a renderer or raster defect.`,
    '',
    ...
    '',
    `Bounded gap reviews: ${report.gapReviewInputs?.observations.length ?? 0} original observations support ${report.discrepancies.filter(d => Object.values(gapReviewAttributions).includes(d.attribution)).length} capture/observation-stage groups. Original motion requests, raw omitted values and full memberships remain intact; ${report.gapReviewInputs?.coverage?.unresolvedObservations ?? 'unbound'} motion observations remain unresolved. Independent source and classification coverage do not establish candidate computed gaps, used spacing, equivalent inputs or renderer causality.`,
    '',
    `Explicit gap composition: ${report.explicitGapInputs?.observations.length ?? 0} original observations support ${report.discrepancies.filter(d => d.attribution === explicitGapAttribution).length} unequal application/plugin spacing groups. Full composition replay and independent source coverage preserve explicit shorthand, raw omitted longhands, all original memberships and prior classifications. Missing proof observations: ${report.explicitGapInputs?.coverage?.missing.length ?? 'unbound'}. This is not computed-gap, layout or rendering equivalence; equivalent-input core reductions remain required before removing the substitutions.`,
    '',
    ...
    `Paginator tooltips: ${report.retainedTypography.gaps.filter(gap => gap.attribution === 'reviewed-paginator-tooltip-omission').length} reference-only tooltip owners retain the above-trigger connected overlay, navigation-button state and complete candidate paginator inputs. The candidate authors no tooltip counterpart; absent content is not a core retained-text failure, equivalent hidden state or verified popup behavior.`,
    '',
    ...
    `Shared button formatting: ${report.buttonFlexInputs.observations.length} independently bound owners retain explicit Material inline-flex/center/center requests versus omitted candidate authoring and block/stretch/flex-start local defaults. ${report.discrepancies.filter(d => d.attribution === buttonFlexAttribution).length} groups receive unequal-authoring attribution only; native-value/label-span composition, used layout and text-centering remain independent.`,
    `Shared button host requests: ${report.buttonHostRequestInputs.observations.length} independently bound owners retain explicit reference relative/min-width:64px/vertical-align:middle requests versus core absolute placement or absent own-stage candidate values. ${report.discrepancies.filter(d => d.attribution === buttonHostRequestAttribution).length} groups retain those authoring differences without inventing computed defaults. Original source rules precede later parity fixes; containing blocks, used layout, composition and raster cause remain unproven.`,
    `Fixed button width authoring: ${report.buttonFixedWidthInputs.observations.length} independently bound owners retain ${report.buttonFixedWidthInputs.groups.length} unequal authored-width groups, including ${report.buttonFixedWidthInputs.observations.filter(o => o.element === 'core-primary').length} core owners whose widths normalize equally. ${report.discrepancies.filter(d => d.attribution === buttonFixedWidthAttribution).length} scalar groups receive attribution; the separate authoring ledger is never filtered by numerical similarity. Used dimensions, intrinsic sizing and raster cause remain unproven.`,
    `Button box-sizing observation stages: ${report.buttonBoxSizingInputs.observations.length} independently source-bound owners retain browser border-box and omitted candidate local declarations, with ${report.buttonBoxSizingInputs.observations.filter(o => o.proof.observedDeclaredBorderBox).length} measured static cases and ${report.buttonBoxSizingInputs.observations.filter(o => !o.proof.observedDeclaredBorderBox).length} original interaction geometry gaps. ${report.discrepancies.filter(d => d.attribution === buttonBoxSizingAttribution).length} groups receive bounded observation-stage attribution. No candidate computed default, full layout, interaction geometry or rendering equivalence is inferred; fixed-width authoring remains independently unequal.`,
    `Field-host layout requests: ${report.fieldHostLayoutInputs.observations.length} independently source-bound hosts retain reference automatic inline-flex column composition versus candidate fixed-height block/absolute-child authoring. ${report.discrepancies.filter(d => d.attribution === fieldHostLayoutAttribution).length} groups receive authored-request attribution, including explicit minimum-width requests previously obscured by a generic omission shortcut; ${report.discrepancies.filter(d => d.attribution === fieldHostWidthAttribution).length} width groups retain identical percentage requests at different observation stages. Static geometry and interaction measurement gaps remain separate; no candidate computed values, whole-input equivalence or original renderer cause is inferred.`,
    '',
    `Gap observation stages: ${report.ownerGapInputs.observations.length} independently source-bound observations retain browser-computed normal versus omitted candidate local longhands. ${report.discrepancies.filter(d => d.attribution === ownerGapAttribution).length} groups receive diagnostic-stage attribution only; ${report.ownerGapInputs.observations.filter(o => o.proof.issues.length).length} explicit-request, motion, mapping or scalar-rule review observations remain retained. Full original-tree replay and independently reconstructed classification coverage are both required. Candidate computed normal/zero, inherited reset context, used spacing, structural equivalence and rendering remain unverified; the public grammar/unit/axis failures are not waived.`,
    ...
    `Grid-template observation stages: ${report.ownerGridInitialInputs.observations.length} independently source-bound original property observations retain browser-computed none and omitted candidate local fields, including ${report.ownerGridInitialInputs.observations.filter(o => o.proof.issues.length).length} review gaps. ${report.discrepancies.filter(d => d.attribution === ownerGridInitialAttribution).length} additional groups receive observation-stage attribution only after prior classifications retain precedence. Earlier non-grid evidence, explicit requests and mapping/motion gaps remain separate. No candidate computed value, implicit-track sizing or rendering equivalence is inferred.`,
...
export function collectStyleDiscrepancies(cases, originStageEvidence, retainedTypography, visibleOverflowInputs, borderInitialInputs, buttonBorderResetInputs, outlineTokenInputs, chipOutlineInputs, nonGridTemplateInputs, buttonTypographyScalarInputs, chipHostTypographyInputs, fieldHostTypographyInputs, rootTypographyInputs, appearanceInitialInputs, buttonAppearanceInputs, rootColorInputs, fieldColorInputs, rootHeightInputs, containerCaretInputs, fieldHostAlignmentInputs, rootInitialStyleInputs, fieldHostWeightTrackingInputs, tooltipUnpairedStyles, sliderInputBoxes, sliderBorderDefaults, fieldHostInitialStyleInputs, ownerInitialStyleEvidence = { observations: [] }, tooltipWrappingInputs = { observations: [] }, rootShadowInputs = { observations: [] }, rootFlowHeightInputs = { observations: [] }, buttonPillRadiusInputs = { observations: [] }, buttonFlexInputs = { observations: [] }, buttonHostRequestInputs = { observations: [] }, buttonFixedWidthInputs = { observations: [] }, ownerGridInitialInputs = { observations: [] }, buttonBoxSizingInputs = { observations: [] }, fieldHostLayoutInputs = { observations: [] }, ownerGapInputs = { observations: [] }, explicitGapInputs = { observations: [], groups: [] }, gapReviewInputs = { observations: [], groups: [] }, ownerCaretInputs = { observations: [] }, reviewedInputs = { observations: [] }, followupInputs = { observations: [] }, alignmentFontInputs = { observations: [] }, textAlignInputs = { observations: [] }, ltrAlignmentInputs = { observations: [] }, reviewedSourceBatchInputs = { observations: [] }, rootBackgroundInputs = { observations: [] }) {
  ...
  const reviewedSourceBatchByCaseIdProperty = reviewedSourceBatchClassificationContexts(reviewedSourceBatchInputs);
  const rootBackgroundByCaseIdProperty = rootBackgroundClassificationContexts(rootBackgroundInputs);
  ...
  const gapReviewByCaseIdProperty = gapReviewClassificationContexts(gapReviewInputs);
  const ownerGapByCaseIdProperty = new Map(ownerGapInputs.observations.map(o => [JSON.stringify([o.case, o.proof.element, o.proof.property]), o]));
  const fieldHostLayoutByCaseId = new Map(fieldHostLayoutInputs.observations.map(o => [JSON.stringify([o.case, o.proof.element]), o]));
  const buttonBoxSizingByCaseId = new Map(buttonBoxSizingInputs.observations.map(o => [JSON.stringify([o.case, o.proof.element]), o]));
  const ownerGridByCaseIdProperty = new Map(ownerGridInitialInputs.observations.map(o => [JSON.stringify([o.case, o.proof.element, o.proof.property]), o]));
  const buttonFixedWidthByCaseId = new Map(buttonFixedWidthInputs.observations.map(p => [JSON.stringify([p.case, p.element]), p]));
  const buttonFlexByCaseId = new Map(buttonFlexInputs.observations.map(p => [JSON.stringify([p.case, p.element]), p]));
  const buttonHostRequestByCaseId = new Map(buttonHostRequestInputs.observations.map(p => [JSON.stringify([p.case, p.element]), p]));
  ...
  const rootHeightByCaseId = new Map(rootHeightInputs.map(p => [JSON.stringify([p.case, p.element]), p]));
  const fieldColorByCaseId = new Map(fieldColorInputs.map(p => [JSON.stringify([p.case, p.element]), p]));
  const rootColorByCaseId = new Map(rootColorInputs.map(p => [JSON.stringify([p.case, p.element]), p]));
  ...
  for (const benchmarkCase of cases) {
    ...
    for (const input of benchmarkCase.styleInputs ?? []) {
      ...
      for (const property of [...properties].sort()) {
        ...
        let classification = benchmarkCase.state && input.reference && input.astylar && input.astylarResolvedStyleEvidenceVersion !== 2
          ...
          : classifyFieldHostLayoutInput(input, property, referenceValue, astylarValue,
              fieldHostLayoutByCaseId.get(JSON.stringify([key, input.id])), canonicalStyle)
            ...
            ?? classifySliderInputBox(input, property, referenceValue, astylarValue,
              sliderBoxByCaseId.get(JSON.stringify([key, input.id])), canonicalStyle)
            ?? classifySliderBorderDefault(input, property, referenceValue, astylarValue,
              sliderBorderByCaseId.get(JSON.stringify([key, input.id])), canonicalStyle)
            ...
            ?? classifyFieldColorInput(input, property, referenceValue, astylarValue,
              fieldColorByCaseId.get(JSON.stringify([key, input.id])), canonicalStyle)
            ?? classifyContainerCaretInput(input, property, referenceValue, astylarValue,
              containerCaretByCaseId.get(JSON.stringify([key, input.id])), canonicalStyle)
            ?? classifyFieldHostAlignmentInput(input, property, referenceValue, astylarValue,
              fieldAlignmentByCaseId.get(JSON.stringify([key, input.id])))
        ...
        if (classification.attribution === 'unresolved') classification = classifyButtonFlexInput(
          input, property, referenceValue, astylarValue,
          buttonFlexByCaseId.get(JSON.stringify([key, input.id])), canonicalStyle) ?? classification;
        if (classification.attribution === 'unresolved') classification = classifyButtonHostRequestInput(
          input, property, referenceValue, astylarValue,
          buttonHostRequestByCaseId.get(JSON.stringify([key, input.id])), canonicalStyle) ?? classification;
        if (classification.attribution === 'unresolved') classification = classifyButtonFixedWidthInput(
          input, property, referenceValue, astylarValue,
          buttonFixedWidthByCaseId.get(JSON.stringify([key, input.id])), canonicalStyle) ?? classification;
        if (classification.attribution === 'unresolved') classification = classifyOwnerGridInitialInput(
          input, property, referenceValue, astylarValue,
          ownerGridByCaseIdProperty.get(JSON.stringify([key, input.id, property]))) ?? classification;
        if (classification.attribution === 'unresolved') classification = classifyButtonBoxSizingInput(
          input, property, referenceValue, astylarValue,
          buttonBoxSizingByCaseId.get(JSON.stringify([key, input.id]))) ?? classification;
        if (classification.attribution === 'unresolved') classification = classifyOwnerGapInput(
          input, property, referenceValue, astylarValue,
          ownerGapByCaseIdProperty.get(JSON.stringify([key, input.id, property]))) ?? classification;
        if (classification.attribution === 'unresolved') classification = classifyExplicitGapComposition(
          input, property, referenceValue, astylarValue,
          explicitGapByCaseIdProperty.get(JSON.stringify([key, input.id, property]))) ?? classification;
        if (classification.attribution === 'unresolved') classification = classifyGapReview(
          input, property, referenceValue, astylarValue,
          gapReviewByCaseIdProperty.get(JSON.stringify([key, input.id, property]))) ?? classification;
        if (classification.attribution === 'unresolved') classification = classifyOwnerCaretInput(
          input, property, referenceValue, astylarValue,
          ownerCaretByCaseIdProperty.get(JSON.stringify([key, input.id, property]))) ?? classification;
        if (classification.attribution === 'unresolved') classification = classifyReviewedInput(
          input, property, referenceValue, astylarValue,
          reviewedInputByCaseIdProperty.get(JSON.stringify([key, input.id, property]))) ?? classification;
        ...
        if (classification.attribution === 'unresolved') classification = classifyReviewedSourceBatchInput(
          input, property, referenceValue, astylarValue,
          reviewedSourceBatchByCaseIdProperty.get(JSON.stringify([key, input.id, property]))) ?? classification;
        ...
        if (!entry) {
          entry = {
            ...
            ...([...reviewedInputAttributions, ...followupInputAttributions, ...alignmentFontAttributions,
              ...textAlignAttributions, ltrAlignmentAttribution, ...reviewedSourceBatchAttributions, rootBackgroundAttribution].includes(classification.attribution) ? { reviewedCases: [] } : {}),
...
function validateCalendarWeekdayEvidence(report, errors) {
  ...
  const replay = key => {
    if (!replays.has(key)) {
      const match = parseReviewedCase(key, 'datepicker');
  ...
  for (const [list, predicate, label = 'calendar weekday'] of [
    ...
    ['comparisons', m => m.element === 'datepicker-month-marker' || m.mapping?.kind === 'reviewed-calendar-month-marker-text', 'calendar month marker'],
    ['differences', d => d.element === 'datepicker-month-marker' || d.attribution === 'reviewed-calendar-month-marker-typography-input', 'calendar month marker'],
  ]) {
...
function reviewedPaginatorTooltipGap(key, inventory) {
  ...
  if (ref?.side !== 'reference' || ast?.side !== 'astylar' || ref.family !== 'paginator' || ast.family !== 'paginator' ||
  ...
  for (const [type, className] of [['div', 'mat-mdc-tooltip'], ['mat-tooltip-component'], ['div', 'mat-mdc-tooltip-panel'],
    ...
    const parent = one(ref.nodes.filter(n => n.key === referenceOverlayPath.at(-1).parent));
    if (!parent || parent.type !== type || (className && !cls(parent, className)) || parent.ownText?.trim() || children(ref, parent).length !== 1) return;
    referenceOverlayPath.push(parent);
  }
  if (!cls(referenceOverlayPath[1], 'mat-mdc-tooltip-show') || referenceOverlayPath[2].attributes['aria-hidden'] !== 'true' ||
      !cls(referenceOverlayPath[3], 'cdk-overlay-pane') || !cls(referenceOverlayPath[3], 'mat-mdc-tooltip-panel-above') ||
      !/^cdk-overlay-\d+$/.test(referenceOverlayPath[3].attributes.id ?? '') || referenceOverlayPath.at(-1).parent !== null ||
  ...
  for (const [type, className, id] of [['div', 'mat-mdc-paginator-range-actions'], ['div', 'mat-mdc-paginator-container'],
    ...
    const parent = one(ref.nodes.filter(n => n.key === referenceTriggerPath.at(-1).parent));
    if (!parent || parent.type !== type || !cls(parent, className) || (id && parent.attributes.id !== id) || parent.ownText?.trim()) return;
    referenceTriggerPath.push(parent);
  }
  if (referenceTriggerPath.at(-1).parent !== null || referenceTriggerPath[4].attributes.role !== 'group') return;
  const candidateNext = one(ast.nodes.filter(n => n.authored?.id === 'paginator-next'));
  const candidatePrevious = one(ast.nodes.filter(n => n.authored?.id === 'paginator-previous'));
  ...
  if (candidatePath.at(-1).parent !== 'root' || candidatePath[3].authored.role !== 'group') return;
  const allowedIds = new Set(['page', 'eyebrow', 'title', 'paginator-root', 'paginator-primary', 'paginator-container',
    'paginator-page-size-group', 'paginator-size', 'paginator-page-size', 'paginator-range-actions', 'paginator-range', 'paginator-previous', 'paginator-next']);
  ...
  return { case: key, family: 'paginator', element: undefined,
    reason: 'own-text nodes without an explicit shared ID require structural mapping', referenceNodes: [leaf.key], astylarNodes: [],
    ...
    reviewEvidence: structuredClone({ sourceFinding: 'fixture-paginator-tooltip-omitted', source: ast.resolvedStyleSource, revision: am.resolvedStyleRevision,
      ...
      candidateTree: ast.nodes.map(n => snapshot(n, 'astylar')), textMappings }) };
}

function isReviewedPaginatorTooltipGap(gap, inventory) {
  if (gap.attribution !== 'reviewed-paginator-tooltip-omission') return false;
  const expected = reviewedPaginatorTooltipGap(gap.case, inventory);
  return expected !== undefined && JSON.stringify(gap) === JSON.stringify(expected);
}

function validatePaginatorTooltipOmissions(report, errors) {
  const inventory = report.elementInventory;
  const expected = [...new Set(inventory.cases.map(c => c.case))].map(key => reviewedPaginatorTooltipGap(key, inventory)).filter(Boolean);
  const claimed = report.retainedTypography?.gaps.filter(g => g.attribution === 'reviewed-paginator-tooltip-omission') ?? [];
  if (expected.length !== claimed.length || claimed.some(g => !isReviewedPaginatorTooltipGap(g, inventory)) ||
...
function validateTimepickerOptionEvidence(report, errors) {
  ...
  const predicate = value => value.kind === 'reviewed-timepicker-option-text' || value.mapping?.kind === 'reviewed-timepicker-option-text' ||
    value.attribution === 'reviewed-timepicker-option-ink-input' ||
...
// Exclusion only: a known mismatching terminal compound cannot target this
// node. Ancestor/sibling relationships and pseudo-state truth are deliberately
// not evaluated. Unknown syntax remains possibly applicable and blocks review.
function typographySelectorCanApply(selector, authored) {
...
function reviewedTimepickerOptionInk(entry, mapping, property, ast, styles, referenceTree, astylarTree, inventory) {
  if (entry.family !== 'timepicker' || property !== 'color' || mapping?.kind !== 'reviewed-timepicker-option-text' ||
      mapping.astylarNode !== ast.key || ast.retainedText?.source !== 'core-text-registry' || referenceTree.ruleEvidenceComplete !== true) return;
  const one = nodes => nodes.length === 1 ? nodes[0] : undefined;
  const leaf = one(referenceTree.nodes.filter(n => n.key === mapping.referenceNode));
  const owner = one(referenceTree.nodes.filter(n => n.key === mapping.reviewEvidence.referenceOption.key));
  if (!leaf || !owner || leaf.parent !== owner.key) return;
  const changesInk = declarations => Object.keys(declarations ?? {}).some(key => {
    const name = key.replaceAll('-', '').toLowerCase();
    return ['color', 'all', 'webkittextfillcolor'].includes(name) || /^(animation|transition)/.test(name);
  });
  ...
  for (const node of [leaf, owner]) {
    ...
    if (pooled?.side !== 'reference' || !pooled.value || typeof pooled.value !== 'object' || Array.isArray(pooled.value) ||
        canonicalStyle(pooled.value).color !== styles.reference.color ||
        unsafeInline(node.inline) || /(?:^|;)\s*(?:color|all|-webkit-text-fill-color|animation[^:]*|transition[^:]*)\s*:/i.test(node.attributes?.style ?? '')) return;
    const rules = node.rules.map(i => inventory.rules[i]);
    if (rules.some(r => r?.side !== 'reference' || !r.value || typeof r.value !== 'object' || Array.isArray(r.value) ||
        typeof r.value.active !== 'boolean')) return;
    const inkRules = rules.map(r => r.value).filter(r => r.active && changesInk(r.declarations));
    if (node === leaf) {
      // This label has no color declaration: inheritance is from its unique
      // direct option parent, whose actual computed value must agree.
      if (inkRules.length) return;
    } else {
      const rule = inkRules[0];
      if (inkRules.length !== 1 || rule.selector !== '.mat-mdc-option' || rule.conditions?.length !== 0 ||
          rule.declarations?.color?.value !== 'var(--mat-option-label-text-color, var(--mat-sys-on-surface))' ||
          rule.declarations.color.important !== false ||
          Object.keys(rule.declarations).some(k => k !== 'color' && changesInk({ [k]: true }))) return;
      referenceRule = rule;
    }
    referenceChain.push({ node: node.key, parent: node.parent, attributes: node.attributes, inline: node.inline,
      computed: pooled.value, rules: rules.map(r => r.value) });
  }
  const pooledRules = astylarTree.rules.map(i => inventory.rules[i]);
  if (pooledRules.some(r => r?.side !== 'astylar' || !r.value || typeof r.value !== 'object' || Array.isArray(r.value))) return;
  const candidateRules = pooledRules.map(r => r.value).filter(r => changesInk(r));
  const applicable = candidateRules.filter(rule => typographySelectorCanApply(rule.selector, ast.authored));
  const ink = 'rgba(29,27,32,1)';
  if (unsafeInline(ast.authored.style) || applicable.length !== 1 || applicable[0].selector !== '.picker-option' ||
      applicable[0].color !== '#1d1b20' || Object.keys(applicable[0]).some(k => k !== 'color' && changesInk({ [k]: true })) ||
      !/^rgba\(/.test(styles.reference.color ?? '') || styles.reference.color === ink ||
      ['normal', 'effective', 'retained'].some(stage => styles[stage].color !== ink)) return;
  return { attribution: 'reviewed-timepicker-option-ink-input', classification: 'application-plugin-authoring-defect',
    inputEquivalent: false, currentPseudoStatePaintVerified: false, finalRasterVerified: false,
    recommendedOwner: 'showcase timepicker option color-token translation and reference text ownership',
    justification: 'The reference label inherits color from its unique Material option parent, whose original active color-token declaration and computed ink are captured with no intervening or competing ink declaration. The replacement direct-text candidate option authors literal #1d1b20, unchanged in normal, effective and retained inputs. This is an unequal authored color input, not a core color conversion or an acceptable near-color substitution. Restore the original token and ownership before testing equal-input color/paint; the token fallback origin, theme scope, state-layer compositing and final raster remain separate obligations.',
    reviewEvidence: { sourceFinding: 'fixture-timepicker-option-ink-substitution', referenceRule, referenceChain,
      candidateRule: applicable[0], checkedCandidateRules: candidateRules,
      candidateAuthored: ast.authored, candidateNormal: inventory.styles[ast.normalStyle].value,
      candidateEffective: inventory.styles[ast.interactionStyle].value, candidateRetained: inventory.styles[ast.retainedText.style].value,
      referenceComputed: styles.reference.color, candidateColor: ink, inputEquivalent: false, finalRasterVerified: false } };
}

function reviewedCalendarMonthMarkerTypography(entry, mapping, property, ast, styles, referenceTree, astylarTree, inventory) {
  if (entry.family !== 'datepicker' || mapping?.kind !== 'reviewed-calendar-month-marker-text' ||
      !['lineHeight', 'textAlign', 'color'].includes(property) || ast.retainedText?.source !== 'core-text-registry' ||
      referenceTree.ruleEvidenceComplete !== true || mapping.astylarNode !== ast.key) return;
  const ref = referenceTree.nodes.find(n => n.key === mapping.referenceNode);
  const cssProperty = property.replace(/[A-Z]/g, letter => '-' + letter.toLowerCase());
  const normalized = property.toLowerCase();
  const changesProperty = declarations => Object.keys(declarations ?? {}).some(key => {
    const name = key.replaceAll('-', '').toLowerCase();
    return name === normalized || name === 'font' || name === 'all' || /^(animation|transition)/.test(name);
  });
  const unsafeInline = value => value !== undefined && (value === null || typeof value !== 'object' || Array.isArray(value) || changesProperty(value));
  if (!ref || changesProperty(ref.inline) || new RegExp(`(?:^|;)\\s*(?:${cssProperty}|font|all|animation[^:]*|transition[^:]*)\\s*:`, 'i').test(ref.attributes?.style ?? '')) return;
  const rules = ref.rules.map(i => inventory.rules[i]);
  if (rules.some(r => r?.side !== 'reference' || !r.value || typeof r.value !== 'object' || Array.isArray(r.value))) return;
  const declarations = rules.map(r => r.value).filter(r => r.active === true && changesProperty(r.declarations));
  const expected = { lineHeight: '0', textAlign: 'start',
    color: 'var(--mat-datepicker-calendar-body-label-text-color, var(--mat-sys-on-surface))' }[property];
  if (declarations.length !== 1 || declarations[0].selector !== '.mat-calendar-body-label' ||
      declarations[0].declarations?.[cssProperty]?.value !== expected ||
      declarations[0].declarations[cssProperty].important !== false || declarations[0].conditions?.length !== 0 ||
      Object.keys(declarations[0].declarations).some(k => k !== cssProperty && changesProperty({ [k]: true })) ||
      (property !== 'color' && styles.reference[property] !== expected)) return;
  const pooled = astylarTree.rules.map(i => inventory.rules[i]);
  if (pooled.some(r => r?.side !== 'astylar' || !r.value || typeof r.value !== 'object' || Array.isArray(r.value))) return;
  const candidateRules = pooled.map(r => r.value).filter(r => changesProperty(r));
  let candidateChain, candidateRule;
  if (property === 'lineHeight') {
    candidateChain = candidateTypographyOmissionChain(ast, astylarTree, inventory, property);
    if (!candidateChain || styles.retained.lineHeight !== 'normal') return;
    for (const item of candidateChain) {
      const nodes = astylarTree.nodes.filter(n => n.key === item.node), node = nodes[0];
      if (nodes.length !== 1 || unsafeInline(node.authored?.style) ||
          ['normal', 'effective'].some(stage => changesProperty(item[stage])) ||
          candidateRules.some(rule => typographySelectorCanApply(rule.selector, node.authored))) return;
    }
  } else {
    const value = property === 'textAlign' ? 'center' : 'rgba(29,27,32,1)';
    const applicable = candidateRules.filter(rule => typographySelectorCanApply(rule.selector, ast.authored));
    if (unsafeInline(ast.authored?.style) || applicable.length !== 1 || applicable[0].selector !== '.datepicker-cell' ||
        Object.keys(applicable[0]).some(k => k !== property && changesProperty({ [k]: true })) ||
        canonicalStyle(applicable[0])[property] !== value || ['normal', 'effective', 'retained'].some(stage => styles[stage][property] !== value)) return;
    candidateRule = applicable[0];
    candidateChain = [{ node: ast.key, normal: inventory.styles[ast.normalStyle].value,
      effective: inventory.styles[ast.interactionStyle].value }];
  }
  return { classification: 'application-plugin-authoring-defect', attribution: 'reviewed-calendar-month-marker-typography-input',
    inputEquivalent: false, currentPseudoStatePaintVerified: false,
    recommendedOwner: 'showcase calendar month-label typography declarations and table-cell structure',
    justification: property === 'lineHeight'
      ? 'The reference month-label cell explicitly authors line-height:0; the complete candidate text-to-page chain and all potentially applicable captured rules omit it, leaving retained normal. This is an omitted input before text layout, not a core conversion of explicit zero to normal. Fixed cell height, flex centering or percentage-padding substitutions do not make the declarations equivalent. No natural metric or final glyph-placement claim follows.'
      : property === 'textAlign'
        ? 'The reference month-label cell explicitly authors text-align:start. The candidate instead authors center on its generic date cell, unchanged in normal, effective and retained inputs. Its separate flex-start justification positions a different kind of child and is not a replacement for the reference text-align declaration. Restore the original text and table-cell inputs before diagnosing core alignment; a similar label position is not input equivalence.'
        : 'The reference month-label ink comes from the original calendar-body-label color token chain; the candidate generic date cell authors literal #1d1b20, retained unchanged by core. The exact source declaration and three candidate stages identify unequal authoring, not a core color conversion or accepted near-color match. Restore the original token before testing equal-input paint.',
    reviewEvidence: { sourceFinding: 'fixture-calendar-month-marker-typography-substitution', property,
      referenceNode: ref.key, referenceAttributes: ref.attributes, referenceInline: ref.inline, referenceRule: declarations[0],
      referenceComputed: styles.reference[property], candidateRule, candidateChain, checkedCandidateRules: candidateRules,
      candidateRetained: styles.retained[property], inputEquivalent: false, finalRasterVerified: false } };
}
...
function reviewedFieldLabelStateColor(entry, wrapper, declarations, referenceTree, astylarTree, inventory) {
  ...
  const states = [
    ...
    ['error', `${base}.mdc-text-field--invalid .mdc-floating-label`,
      'var(--mat-form-field-filled-error-label-text-color, var(--mat-sys-error))'],
  ];
  ...
  if (classes.includes('mdc-text-field--disabled') !== (state[0] === 'disabled') ||
      classes.includes('mdc-text-field--focused') !== (state[0] === 'focus') ||
      classes.includes('mdc-text-field--invalid') !== (state[0] === 'error')) return;
...
function reviewedFieldLabelColorInput(entry, ref, ast, styles, referenceTree, astylarTree, inventory) {
  ...
  return { attribution: 'reviewed-field-label-color-substitution', classification: 'application-plugin-authoring-defect',
    ...
    justification: 'The reference mat-label inherits its active filled-label color token from its floating-label wrapper. For focus, hover, disabled and unfocused error colors, exact captured token declarations and filled-field ancestor state are preserved. Candidate literal base, empty-state and picker-shell colors replace those tokens; its earlier disabled literal is overridden by the base rule, and later picker-shell rules can override base/empty error ink. Captured classes and original rule order select the reviewed declaration, whose color agrees with normal, effective and retained values. These inputs differ before rendering; no core color-conversion defect or equivalent glyph paint is inferred. Stage disagreement, competing reference rules or unreviewed state/media rules prevent attribution. Preserve wrapper, visibility and state inputs before evaluating rendering.',
...
function reviewedFloatingLabelInput(entry, ref, ast, styles, referenceTree, astylarTree, inventory, mode = 'floating') {
  ...
  if (hidden ? wrapperStyle.display !== 'none' || wrapperStyle.transform !== 'none' || wrapperStyle.transformOrigin !== '0% 0%' : floating ? wrapperStyle.transformOrigin !== '0px 0px' ||
    ...
    wrapperStyle.display === 'none' ? wrapperStyle.transform !== 'none' || wrapperStyle.transformOrigin !== '0% 0%' :
      ...
      !/^matrix\(1,\s*0,\s*0,\s*1,\s*0,\s*-?(?:\d+(?:\.\d+)?|\.\d+)\)$/.test(wrapperStyle.transform ?? '')) return;
  ...
  let referenceErrorState;
  if (!floating) {
    if (!isEmpty) return;
    const colorRules = referenceChain[1].rules.filter(r => r.declarations.color || r.declarations.all);
    const state = reviewedFieldLabelStateColor(entry, wrapper, colorRules, referenceTree, astylarTree, inventory);
    if (state?.state !== 'error') return;
    const input = one(referenceTree.nodes.filter(n => n.type === 'input' && n.attributes?.id === `${entry.family}-control`));
    const candidateInput = one(astylarTree.nodes.filter(n => n.authored?.type === 'input' && n.authored.id === `${entry.family}-control`));
    const candidateRegion = one(astylarTree.nodes.filter(n => n.key === candidateInput?.parent));
    const shell = one(astylarTree.nodes.filter(n => n.key === ast.parent));
    if (!input || input.parent !== wrapper.parent || wrapper.attributes.for !== input.attributes.id ||
        input.attributes['aria-invalid'] !== 'true' || input.value !== '' || Object.hasOwn(input.attributes, 'disabled') ||
        candidateInput?.authored.ariaInvalid !== true || candidateInput.authored.value !== '' || candidateInput.authored.disabled === true ||
        ast.authored.for !== candidateInput.authored.id || candidateRegion?.parent !== shell?.key ||
        candidateRegion?.authored?.type !== 'div' || candidateRegion.authored.id !== `${entry.family}-input-region` ||
        shell?.authored?.type !== 'div' || shell.authored.id !== `${entry.family}-primary`) return;
    referenceErrorState = { state, input, candidateInput, candidateRegion, candidateShell: shell,
      referenceWrapperDisplay: wrapperStyle.display, referenceWrapperTransform: wrapperStyle.transform };
  }
  ...
  for (const entry of cases) {
    ...
    if (anonymousReference.length || anonymousAstylar.length) {
      ...
      for (const referenceNode of anonymousReference) {
        ...
        const paginatorTooltip = entry.family === 'paginator' && anonymousAstylar.length === 0 && reviewedPaginatorTooltipGap(key, inventory);
        if (paginatorTooltip?.referenceNodes?.[0] === referenceNode) { gaps.push(paginatorTooltip); continue; }
    ...
    for (const id of ids) {
      ...
      for (const property of retainedTypographyProperties) {
        ...
        if (values.reference === undefined || values.retained === undefined) {
          ...
          const monthMarkerToken = reviewedCalendarMonthMarkerTypography(entry, textMappingById.get(id), property, ast, styles, referenceTree, astylarTree, inventory);
  ...
  for (const entry of cases) {
    ...
    for (const node of paintedNodes) if (!mapped.has(node.key)) {
      ...
      if (substitution) iconSubstitutions.push(substitution);
        ...
        const stateDivergence = reviewedCalendarCloseStateControl(entry, node, referenceTree, astylarTree);
        if (stateDivergence) gaps.push({ case: key, element: node.authored.id, astylarNode: node.key,
          reason: 'current control texture has no reviewed reference text-owner mapping', ...stateDivergence });
...
function reviewedCalendarCloseStateControl(entry, node, referenceTree, astylarTree) {
  const match = parseReviewedCase(caseKey(entry), 'datepicker');
  if (match?.[1] !== 'supplemental' || !entry.state.endsWith('-activate-close') || entry.action !== 'Enter' ||
      entry.reference?.open !== false || entry.reference?.close !== null || entry.reference?.openerFocused !== true ||
      entry.astylar?.open !== true || entry.astylar?.close !== null || entry.inputEquivalent !== false || entry.finalRasterVerified !== false ||
      !entry.reference.events?.some(e => e.type === 'keydown' && e.key === 'Enter' && e.close && e.trusted) ||
      !entry.reference.events?.some(e => e.type === 'click' && e.close && e.trusted)) return;
  if (referenceTree.nodes.some(n => ['mat-calendar', 'mat-datepicker-content'].includes(n.type) ||
      String(n.attributes?.class ?? '').split(/\s+/).includes('mat-datepicker-close-button'))) return;
  if (node.authored?.type !== 'button' || !/^datepicker-(?:month|previous|next|(?:day|year)-\d+)$/.test(node.authored.id ?? '') ||
      node.paintedControlText?.source !== 'core-control-texture' || typeof node.authored.value !== 'string' ||
      node.authored.value !== node.paintedControlText.text ||
      astylarTree.nodes.filter(n => n.authored?.id === node.authored.id).length !== 1) return;
  const popups = astylarTree.nodes.filter(n => n.authored?.id === 'datepicker-popup' && n.authored.role === 'dialog');
  if (popups.length !== 1 || new Set(astylarTree.nodes.map(n => n.key)).size !== astylarTree.nodes.length) return;
  if (astylarTree.nodes.some(n => Object.values(n.authored ?? {}).some(value => typeof value === 'string' &&
      /close\s+calendar|datepicker-close/i.test(value)))) return;
  const path = [], visited = new Set();
  let current = node;
  while (current && !visited.has(current.key)) {
    visited.add(current.key); path.push(current.key);
    if (current === popups[0]) break;
    current = astylarTree.nodes.find(n => n.key === current.parent);
  }
  if (current !== popups[0]) return;
  return { classification: 'application-plugin-authoring-defect', attribution: 'reviewed-calendar-close-state-divergence',
    inputEquivalent: false, finalRasterVerified: false,
    recommendedOwner: 'showcase calendar authored close control and dismissal state',
    reviewEvidence: { sourceFinding: 'fixture-calendar-close-control-omitted', referenceOpen: false, candidateOpen: true,
      referenceOpenerFocused: true, action: 'Enter', candidatePopup: popups[0].key, candidatePath: path,
      authoredControl: structuredClone(node.authored), currentText: node.paintedControlText.text,
      source: node.paintedControlText.source },
    justification: 'The bound keyboard sequence closes and removes the reference calendar, while the candidate has no authored close counterpart and its popup remains open. This current control texture belongs to that remaining candidate popup, so no corresponding reference text owner exists in this state. The mismatch is unequal authored interaction/state, not missing core text paint, typography equivalence or permission to omit the candidate subtree.' };
}
...
function sourceFingerprints(root) {
  const files = [
    ...
    'tests/material-parity/reviewed-source-batch-observation-binding.mjs',
    'tests/material-parity/reviewed-source-batch-observation-binding.spec.mjs',
    'tests/material-parity/reviewed-source-batch-transition.mjs',
    'tests/material-parity/reviewed-source-batch-transition.spec.mjs',
    'tests/material-parity/reviewed-source-batch.spec.mjs',
    'tests/material-parity/reviewed-batch-motion-replay.mjs',
    'tests/material-parity/motion-source-conservation.mjs',
    ...
    'tests/material-parity/alignment-adapter-receipt-source.mjs',
    'tests/material-parity/alignment-adapter-receipt-source.spec.mjs',
    ...
    'tests/material-parity/vertical-align-canonical-plan.spec.mjs',
    'tests/material-parity/text-align-canonical-plan.spec.mjs',
    'tests/material-parity/additional-control-font-style-attribution.spec.mjs',
    ...
    'tests/material-parity/followup-input-canonical-integration.spec.mjs',
    'scripts/bind-material-followup-input-proposals.mjs',
    'scripts/check-material-followup-input-transition.mjs',
    'scripts/audit-material-leaf-font-family-stages.mjs',
    'scripts/audit-material-leaf-font-family-attribution.mjs',
    'scripts/audit-material-leaf-weight-tracking-stages.mjs',
    'scripts/audit-material-leaf-weight-tracking-attribution.mjs',
    'scripts/audit-material-expansion-owner-mapping.mjs',
    'scripts/audit-material-expansion-owner-attribution.mjs',
    'scripts/audit-material-control-font-style-reset.mjs',
    'scripts/audit-material-control-font-style-attribution.mjs',
    'tests/material-parity/leaf-font-family-stages.spec.mjs',
    'tests/material-parity/leaf-font-family-attribution.spec.mjs',
    'tests/material-parity/leaf-weight-tracking-stages.spec.mjs',
    'tests/material-parity/leaf-weight-tracking-attribution.spec.mjs',
    'tests/material-parity/expansion-owner-mapping.spec.mjs',
    'tests/material-parity/expansion-owner-attribution.spec.mjs',
    'tests/material-parity/control-font-style-reset.spec.mjs',
    'tests/material-parity/control-font-style-attribution.spec.mjs',
    'docs/material-leaf-font-family-stages.json',
    'docs/material-leaf-font-family-attribution-plan.json',
    'docs/material-leaf-weight-tracking-stages.json',
    'docs/material-leaf-weight-tracking-attribution-plan.json',
    'docs/material-expansion-owner-mapping.json',
    'docs/material-expansion-owner-attribution-plan.json',
    'docs/material-control-font-style-reset.json',
    'docs/material-control-font-style-attribution-plan.json',
    'docs/material-followup-input-proposal-binding.json',
    'docs/material-followup-input-transition-dry-run.json',
    'tests/material-parity/reviewed-input-audit-source-binding.mjs',
    'tests/material-parity/reviewed-input-audit-source-binding.spec.mjs',
    'tests/material-parity/reviewed-input-proposal-transition.mjs',
    'tests/material-parity/reviewed-input-proposal-transition.spec.mjs',
    'tests/material-parity/reviewed-input-proposal-binding.spec.mjs',
    ...
    'scripts/audit-material-range-font-reset.mjs',
    'scripts/audit-material-slider-disabled-inputs.mjs',
    'scripts/audit-material-authoring-input-attribution.mjs',
    'scripts/audit-material-font-scope-inputs.mjs',
    'scripts/audit-material-expansion-title-inputs.mjs',
    'scripts/audit-material-tab-panel-inputs.mjs',
    'scripts/audit-material-overlay-font-inputs.mjs',
    'scripts/audit-material-font-ownership-attribution.mjs',
    'scripts/audit-material-button-hover-composition.mjs',
    'scripts/audit-material-button-paint-attribution.mjs',
    'scripts/audit-material-host-font-token-inputs.mjs',
    'scripts/audit-material-container-font-family-stages.mjs',
    'docs/material-reviewed-input-proposal-binding.json',
    'docs/material-reviewed-input-transition-dry-run.json',
    'docs/material-container-font-stage-plan.json',
    'docs/material-leaf-font-attribution-plan.json',
    'docs/material-authoring-input-attribution-plan.json',
    'docs/material-font-ownership-attribution-plan.json',
    'docs/material-button-paint-attribution-plan.json',
    'docs/material-host-font-token-attribution-plan.json',
    'docs/material-container-font-family-attribution-plan.json',
    'docs/material-container-font-stages.json',
    'docs/material-leaf-font-stages.json',
    'docs/material-container-font-inputs.json',
    'docs/material-range-font-reset.json',
    'docs/material-slider-disabled-inputs.json',
    'docs/material-font-scope-inputs.json',
    'docs/material-expansion-title-inputs.json',
    'docs/material-tab-panel-inputs.json',
    'docs/material-overlay-font-inputs.json',
    'docs/material-button-hover-composition.json',
    'docs/material-host-font-token-inputs.json',
    'docs/material-container-font-family-stages.json',
    ...
    'examples/material-showcase/src/app/font-relative-box-audit.spec.ts',
    'examples/material-showcase/src/app/identity-transform-context-audit.spec.ts',
    'src/app/services/dom/positioning/stacking-context.manager.ts',
    'docs/compatibility/capabilities.json',
    'examples/material-showcase/node_modules/astylarui/dist/lib/lib/index.js',
    'examples/material-showcase/node_modules/astylarui/dist/lib/app/services/dom/elements/element-creation.service.js',
    'examples/material-showcase/node_modules/astylarui/dist/lib/app/services/dom/elements/css-transform.js',
    'examples/material-showcase/node_modules/astylarui/dist/lib/app/services/dom/positioning/stacking-context.manager.js',
    'examples/material-showcase/src/app/appearance-input-audit.spec.ts',
    'src/app/services/dom/input/checkbox.manager.ts',
    'src/app/services/dom/input/select.manager.ts',
    'src/app/services/dom/elements/element-creation.service.ts',
    'src/app/services/dom/elements/grid.service.ts',
    'src/app/services/dom/elements/grid-track-sizing.ts',
    ...
    'scripts/audit-material-supplemental-line-boxes.mjs',
    'tests/material-parity/supplemental-line-box-evidence.mjs',
    ...
    'examples/material-showcase/node_modules/@angular/material/fesm2022/paginator.mjs',
    ...
    'tests/material-parity/control-line-box-normalization.mjs',
    ...
    'scripts/audit-material-line-box-normalization.mjs',
    'docs/material-line-box-normalization-census.json',
    'tests/material-parity/supplemental-line-box-report.mjs',
    'tests/material-parity/supplemental-line-box-fixtures.mjs',
    'tests/material-parity/supplemental-capture-fixtures.mjs',
    ...
    'tests/material-parity/caret-normalization-transition.spec.mjs',
    ...
    'tests/material-parity/slider-input-box-integration.spec.mjs',
...
function focusedProofInventory(root) {
  return [
    ...
    proof(root, 'tests/material-parity/owner-grid-initial-canonical-integration.spec.mjs', /test\('owner grid production integration preserves/,
    ...
    proof(root, 'tests/material-parity/button-fixed-width-canonical-integration.spec.mjs', /test\('button fixed widths production integration preserves/,
      'fixed button widths retain authored differences even where scalars match', 'Original source binding retains every selected button owner before numerical filtering. Actual prior production classification supplies the baseline with unchanged normalization and aliases. All nine authored-width groups remain in the ledger, including the scalar-matching core owners; only unequal scalar rows receive attribution. Exact values, existing unrelated classifications and source/state coverage are preserved. Missing binding or lost scalar-matching owners is rejected. Full-report conservation and used-layout/raster proof remain separate gates.'),
    proof(root, 'tests/material-parity/button-requests-canonical-integration.spec.mjs', /test\('button requests production integration preserves/,
    ...
    proof(root, 'tests/material-parity/slider-input-box-integration.spec.mjs', /test\('slider box canonical integration uses/,
    ...
    proof(root, 'examples/material-showcase/src/app/appearance-input-audit.spec.ts', /describe\('Material audit: appearance ownership/,
      'public-package appearance default and indicator-sensitivity reduction', 'Two eight-test runs cover 24 fresh mounts each at DPR1. Six empty non-widget types preserve output size and byte-identical Astylar variant rasters; select and checked-checkbox none variants change indicator pixels. Browser computed keywords and candidate local omissions remain separate. This proves variant sensitivity, not native-widget, plugin, dynamic-state, typography or cross-engine screenshot parity.'),
    ...
    proof(root, 'tests/material-parity/input-tree-evidence.spec.mjs', /test\('matching section boxes/,
      'fixed versus automatic section height browser sensitivity', 'At DPR1/2 identical initial 258x98 border boxes diverge to heights 138/98 after the same child grows and restore after shrink. This proves unequal height requests, not an equal-input Astylar core defect or final raster equivalence.'),
    proof(root, 'tests/material-parity/input-tree-evidence.spec.mjs', /test\('explicit host alignment/,
      'explicit alignment versus inherited request browser sensitivity', 'At DPR1/2 an explicit left-aligned host remains left when its ancestor changes to center/right, whereas the omitted host inherits those values; restoring the ancestor restores initial left/start values. This distinguishes authored requests, not current candidate computed alignment or descendant raster.'),
    proof(root, 'tests/material-parity/input-tree-evidence.spec.mjs', /test\('container caret computed/,
      'container caret declaration versus descendant computed ownership', 'At DPR1/2 omitted ancestor caret declarations compute to container ink while an input with different ink computes a different caret color. An explicit ancestor caret color inherits until the input overrides it with auto; restoring omissions restores the observations. This is browser computed-style sensitivity, not candidate caret paint or visibility proof.'),
    proof(root, 'tests/material-parity/input-equivalence-audit.spec.mjs', /test\('root color separates/,
      'section inherited-color request and local diagnostic-stage separation', 'Complete frame/page ancestry plus independently checked original color and same-sheet dark override establish omitted local section color versus inherited browser computed color. Competing, layered, important, reversed, scalar-stage and forged evidence is rejected. No computed candidate color, caret color, currentColor paint or final raster is inferred.'),
    proof(root, 'tests/material-parity/input-equivalence-audit.spec.mjs', /test\('field host color joins/,
      'field host inherited-color request with independently joined ancestry', 'Exact root-color ancestry and independently proven field-host identity are joined before checking local host omission, inherited browser color and all candidate declaration stages. Competing requests, changed identity/ancestry, scalar changes and forged evidence reject attribution. Missing host font tokens and actual descendant control/caret/currentColor paint remain separate.'),
    ...
    proof(root, 'tests/material-parity/input-equivalence-audit.spec.mjs', /test\('calendar month marker typography traces/,
      'month-label declaration and retained-stage attribution with conflict and replay controls', 'The exact active reference label rule supplies zero line-height, start alignment and a color token. The candidate omits line-height through its full text ancestry or authors center/literal ink unchanged by core. Complete candidate rules are conservatively checked, including potentially applicable state/media rules and unknown selectors. Changed declarations, ancestry, stages and forged reports fail attribution; definitely unrelated terminal selectors do not. This proves unequal input provenance, not general selector matching, natural line metrics or final raster equivalence.'),
...
function parseReviewedCase(key, family) {
  ...
  return family === 'datepicker' && match[2] === 'light' && /^calendar-close-desktop-dpr[12]$/.test(match[3]) &&
    /^calendar-close-(?:month|multi-year)-(?:opened|tab-close|blur-close|refocus-close|activate-close)$/.test(match[4] ?? '') ? match : null;
}
