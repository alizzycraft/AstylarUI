...
const proof = restorePositionProducer(currentSource);
function sample() {
  const previous = { rows: Array.from({ length: 6 }, (_, i) => ({ family: 'fixture', element: `owner-${i}`,
    occurrences: i === 0 ? 56 : 52, attribution: 'unresolved', reference: 'static' })),
  ...
  for (const row of current.rows) row.attribution = positionCompositionAttribution;
...
test('position comparison rejects lost or changed evidence and forged source receipts', () => {
  ...
  ]) { const args = sample(); mutate(args); assert.throws(() => comparePositionCanonical(...args)); }
});
