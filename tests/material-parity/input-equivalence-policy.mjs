...
export const sourceAuditDefinitions = Object.freeze([
  ...
  Object.freeze({
    ...
    justification: 'The unchanged calc(80px) and calc(50% +/- 0.5px) grid-list reductions fail before projection: dimensions retain parent sizes and the second tile left resolves to zero. Direct StyleRule function evaluation is explicitly outside the current contract; the catalog recommends loaded document styles for browser-resolved supported values. Keep the limitation visible and verify that core-owned path for equivalent Material inputs, or scope general CSS expression support separately. Do not introduce a per-fixture arithmetic resolver or treat literal controls as acceptance of the original expressions.',
